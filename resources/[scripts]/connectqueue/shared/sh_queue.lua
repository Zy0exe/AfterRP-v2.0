...
Queue.MaxPlayers = GetConvarInt('sv_maxclients', 150)
...
Queue.InitHostName = Queue.InitHostName ~= "AFTER V2.0" and Queue.InitHostName or false
...
function Queue:DebugPrint(msg)
    if Queue.Debug then
        msg = "^3LISTA DE ESPERA: ^0" .. tostring(msg) .. "^7"
        print(msg)
    end
end
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        done(Config.Language.connectingerr .."\n🔸 Verifique nosso Discord para mais informações: https://discord.gg/qGKrJj7NuM")
...
local function playerConnect(source, setKickReason, deferrals)
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            -- prevent joining
            allow = false
            done(reason and tostring(reason) or "Você foi impedido de entrar")
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
            Queue:DebugPrint(string_format("%s[%s] foi impedido de entrar; Razão: %s", name, ids[1], reason))
            CancelEvent()
            return
        end
