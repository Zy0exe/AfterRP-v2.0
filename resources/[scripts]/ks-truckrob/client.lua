...
AddEventHandler("ks-TruckRobbery:hackertje",function()
	...
	--Citizen.Wait(4000)
	ZyoCore.Functions.Progressbar("hack", "Insira o cartão SD no tablet..", 8500, false, true, {
...
function HackingMinigame(success)
    ...
    if success then
		...
		print('deu')
		ZyoCore.Functions.Notify("Você invadiu o sistema", "error")
...
AddEventHandler("ks-TruckRobbery:startMission",function(spot)
	...
	if numy == 100 then
	    ZyoCore.Functions.Notify("Sem missões disponíveis!")
	else
		CurrentEventNum = num
		TriggerEvent("ks-TruckRobbery:startTheEvent",num)
		PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
		print('deu2 a cena do caminhão')
		ZyoCore.Functions.Notify("O caminhão está marcado no seu GPS!")
	end
end)
	...
	while not taken do
		...
		if missionInProgress == true then
			...
			if distance <= 30.0  then
				if KillGuardsText == false then
					ZyoCore.Functions.Notify("Mate os guardas!")
					KillGuardsText = true
				end
			end
			...
			if distance <= 5 and TruckIsDemolished == false then
			    local requiredItems = {
				    [1] = {name = ZyoCore.Shared.Items["tijdbom"]["name"], image = ZyoCore.Shared.Items["tijdbom"]["image"]},
			    }
		...
		if TruckIsExploded == true then
		    if openText == false then
				ZyoCore.Functions.Notify("Pressione E para roubar tudo :)!")
				openText = true
			end
			local pos = GetEntityCoords(GetPlayerPed(-1), false)
			local TruckPos = GetEntityCoords(ArmoredTruckVeh) 
		...
		if missionCompleted == true then
			ZyoCore.Functions.Notify("Missão completa")
...
AddEventHandler("ks-TruckRobbery:UsableItem",function()
	...
	if missionInProgress == true then
    if distance <= 5 and TruckIsDemolished == false then				 
			ZyoCore.Functions.TriggerCallback('ks-TruckRobbery:server:Hasbomb', function(HasItem)
				if HasItem then
				    TriggerServerEvent("ks-TruckRobbery:RemoveItem")
					BlowTheTruckDoor()
					Citizen.Wait(500) 
				else
					ZyoCore.Functions.Notify('Você precisa de uma bomba-relógio ..', 'error')
				end
			end)
		end
	end
...
function BlowTheTruckDoor()
	if IsVehicleStopped(ArmoredTruckVeh) then
		if (IsVehicleSeatFree(ArmoredTruckVeh, -1) and IsVehicleSeatFree(ArmoredTruckVeh, 0) and IsVehicleSeatFree(ArmoredTruckVeh, 1)) then
			...
			ZyoCore.Functions.Progressbar("hack", "Coloque a bomba", 5500, false, true, {
			...
			ZyoCore.Functions.Progressbar("hack", "Espere..", 9500, false, true, {
			...
			ZyoCore.Functions.Notify('Roubo de caminhão iniciado..', 'error')
		else
			ZyoCore.Functions.Notify('Seguranças ainda estão vivos..', 'error')
		end
	else
		ZyoCore.Functions.Notify('O caminhão ainda não parou ..', 'error')
	end
end
...
function RobbingTheMoney()
	...
	ZyoCore.Functions.Progressbar("hack", "Agarre com suas garras..", 6000, false, true, {
