...
ZyoCore.Functions.CreateCallback('ks-TruckRobbery:server:Hasbomb', function(source, cb)
...
ZyoCore.Functions.CreateUseableItem("sdkaart", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
    ...
    local Player = ZyoCore.Functions.GetPlayer(src)
      Player.Functions.RemoveItem('sdkaart', 1)
        ...
        TriggerClientEvent('ZyoCore:Notify', source, "Você precisa de um tablet..", "error")
    end
end)
...
ZyoCore.Functions.CreateUseableItem("tijdbom", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
...
AddEventHandler('ks-TruckRobbery:missionComplete', function()
    ...
    for i = 1, rolls do
        ...
        for i = 1, math.random(3, 4), 1 do
            ...
            TriggerClientEvent('ZyoCore:Notify', src, 'Garimpeiro..', 'success')
            Player.Functions.AddItem(randItem, amount)
            TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items[randItem], 'add')
            Citizen.Wait(500)
			local luck = math.random(10, 80)
			if luck <= 10 then
			    Player.Functions.AddItem("bluechip", math.random(1, 2))
				TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items["bluechip"], "add")
			end
