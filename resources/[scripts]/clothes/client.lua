	...
	elseif hasToPay then
		ZyoCore.Functions.GetPlayerData(function(PlayerData)
			...
			if cashBalance >= 100 or cashBalance >= 100 then
				...
				ZyoCore.Functions.Notify("You don\'t have enough money! (€100,-)")
			end
		end)
	end
end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if nearcloth < 5.0 and cmenu.show == 0 and ZyoCore ~= nil then
			local pos = GetEntityCoords(GetPlayerPed(-1))
			ZyoCore.Functions.DrawText3D(pos.x, pos.y, pos.z, "~g~E~w~ - To change your clothes / ~g~H~w~ - To save your clothes")
