                    ...
                    if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 7.5) then
                        if onDuty then
                            ...
                            if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 1.5) then
                                ...
                                if IsControlJustReleased(0, Keys["E"]) then
                                    if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                                        --ZyoCore.Functions.DeleteVehicle(GetVehiclePedIsIn(GetPlayerPed(-1)))--eu comentei isto
                                    else
                                        MenuImpound()
                                        currentGarage = k
                                        Menu.hidden = not Menu.hidden
                            ...
                            if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 1.5) then
                                ...
                                if IsControlJustReleased(0, Keys["E"]) then
                                    if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                                        ...
                                        ZyoCore.Functions.SpawnVehicle(Config.Helicopter, function(veh)
                    ...
                    if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 4.5) then
                        if onDuty then
                            if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 1.5) then
                                ...
                                if IsControlJustReleased(0, Keys["E"]) then
                                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "policestash_"..ZyoCore.Functions.GetPlayerData().citizenid)
                                    TriggerEvent("inventory:client:SetCurrentStash", "policestash_"..ZyoCore.Functions.GetPlayerData().citizenid)
                                end
                            elseif (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 2.5) then
                            ...
                            if (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 1.5) then
                                ...
                                if IsControlJustReleased(0, Keys["E"]) then
                                    ...
                                    if player ~= -1 and distance < 2.5 then
                                        ...
                                        ZyoCore.Functions.Notify("Ninguem por perto!", "error")
                                    end
                                end
                            elseif (GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 2.5) then
    ...
    ZyoCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "police" then
            ...
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                ZyoCore.Functions.TriggerCallback('police:GetPlayerStatus', function(result)
                    if result ~= nil then
...
function ImpoundVehicleList()
    ZyoCore.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        ped = GetPlayerPed(-1);
        MenuTitle = "Veiculos:"
        ClearMenu()

        if result == nil then
            ZyoCore.Functions.Notify("Não existe veiculos apreendidos", "error", 5000)
            closeMenuFull()
        else
            for k, v in pairs(result) do
                ...
                Menu.addButton(ZyoCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutImpound", v, "Apreendido", " Motor: " .. enginePercent .. "%", " Chassi: " .. bodyPercent.. "%", " Gasolina: "..currentFuel.. "%")
            end
        end
...
function OutfitsLijst()
    ZyoCore.Functions.TriggerCallback('apartments:GetOutfits', function(outfits)
        ...
        if outfits == nil then
            ZyoCore.Functions.Notify("Não tens nenhuma farda guardada...", "error", 3500)
            closeMenuFull()
        else
...
function selectOutfit(oData)
    ...
    ZyoCore.Functions.Notify(oData.outfitname.." selecionado", "success", 2500)
    closeMenuFull()
    changeOutfit()
end
...
function removeOutfit(oData)
    ...
    ZyoCore.Functions.Notify(oData.outfitname.." removido", "success", 2500)
    closeMenuFull()
end
...
function TakeOutVehicle(vehicleInfo)
    ...
    ZyoCore.Functions.SpawnVehicle(vehicleInfo, function(veh)
...
function IsArmoryWhitelist()
    local retval = false
    local citizenid = ZyoCore.Functions.GetPlayerData().citizenid
