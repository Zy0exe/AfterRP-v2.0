...
AddEventHandler('ZyoCore:Client:OnPlayerLoaded', function()
    ...
    onDuty = ZyoCore.Functions.GetPlayerData().job.onduty
    isHandcuffed = false
    TriggerServerEvent("ZyoCore:Server:SetMetaData", "ishandcuffed", false)
    TriggerServerEvent("police:server:SetHandcuffStatus", false)
    TriggerServerEvent("police:server:UpdateBlips")
    TriggerServerEvent("police:server:UpdateCurrentCops")
    TriggerServerEvent("police:server:CheckBills")

    if ZyoCore.Functions.GetPlayerData().metadata["tracker"] then
...
AddEventHandler('police:client:sendBillingMail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        local charinfo = ZyoCore.Functions.GetPlayerData().charinfo
...
AddEventHandler('police:client:SendPoliceEmergencyAlert', function()
    ...
    TriggerServerEvent("police:server:SendPoliceEmergencyAlert", streetLabel, pos, ZyoCore.Functions.GetPlayerData().metadata["callsign"])
    TriggerServerEvent('qb-policealerts:server:AddPoliceAlert', {
        ...
        details = {
            [1] = {
                ...
                detail = MyId .. ' | ' .. ZyoCore.Functions.GetPlayerData().charinfo.firstname .. ' ' .. ZyoCore.Functions.GetPlayerData().charinfo.lastname,
            },
            [2] = {
...
AddEventHandler('police:client:GunShotAlert', function(streetLabel, isAutomatic, fromVehicle, coords, vehicleInfo)
    if PlayerJob.name == 'police' and onDuty then        
        ...
        local blipText = "Tiros Disparados"
        local MessageDetails = {}
        if fromVehicle then
            blipText = "Tiros Disparados num veiculo"
            MessageDetails = {
                [1] = {
...
AddEventHandler('police:server:SendEmergencyMessageCheck', function(MainPlayer, message, coords)
    ...
    if ((PlayerData.job.name == "police" or PlayerData.job.name == "ambulance" or PlayerData.job.name == "doctor") and onDuty) then
        ...
        SetBlipAsShortRange(blip, false)
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("112 ALERTA")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
