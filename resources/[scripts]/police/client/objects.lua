...
AddEventHandler('police:client:spawnCone', function()
    ZyoCore.Functions.Progressbar("spawn_object", "A colocar objeto..", 2500, false, true, {
...
AddEventHandler('police:client:deleteObject', function()
    ...
    if dist < 5.0 then
        ZyoCore.Functions.Progressbar("remove_object", "Removing object..", 2500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
...
AddEventHandler('police:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 < SpikeConfig.MaxSpikes then
        ...
        ZyoCore.Functions.Notify('Não existe mais linhas de espetos disponiveis...', 'error')
    end
end)
