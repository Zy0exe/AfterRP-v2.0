...
AddEventHandler("evidence:client:ClearBlooddropsInArea", function()
	...
	local blooddropList = {}
	ZyoCore.Functions.Progressbar("clear_blooddrops", "A limpar o sangue...", 5000, false, true, {
		...
		if Blooddrops ~= nil and next(Blooddrops) ~= nil then 
			...
			ZyoCore.Functions.Notify("Limpaste o sangue")
		end
    end, function() -- Cancel
...
AddEventHandler("evidence:client:ClearCasingsInArea", function()
	local pos = GetEntityCoords(GetPlayerPed(-1))
	local casingList = {}
	ZyoCore.Functions.Progressbar("clear_casings", "A remover capsulas de balas...", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
		...
		if Casings ~= nil and next(Casings) ~= nil then 
			...
			ZyoCore.Functions.Notify("Removeste todas as capsulas de balas :)")
		end
    end, function() -- Cancel
        ZyoCore.Functions.Notify("Cancelado", "error")
    end)
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, true) < 1.5 then
				...
				if IsControlJustReleased(0, Keys["G"]) then
					...
					local info = {
						...
						ammolabel = Config.AmmoLabels[ZyoCore.Shared.Weapons[Casings[CurrentCasing].type]["ammotype"]],
		...
		if isArmed then
			if PlayerJob.name ~= "police" then
				if IsPedShooting(GetPlayerPed(-1)) and not IsSilentWeapon(currentWeapon) and IsPedNearby() then
					...
					if ZyoCore.Shared.Weapons[currentWeapon]["ammotype"] ~= "AMMO_PISTOL" then
						automatic = true
					end
...
function IsSilentWeapon(weapon)
	...
	if not retval then 
		ZyoCore.Functions.TriggerCallback('police:IsSilencedWeapon', function(result)
			retval = result
			return result
		end, weapon)
		Citizen.Wait(100)
