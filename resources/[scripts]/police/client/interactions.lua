Citizen.CreateThread(function()
    while true do 
        ...
        if isHandcuffed then
            ...
            if (not IsEntityPlayingAnim(GetPlayerPed(-1), "mp_arresting", "idle", 3) and not IsEntityPlayingAnim(GetPlayerPed(-1), "mp_arrest_paired", "crook_p2_back_right", 3)) and not ZyoCore.Functions.GetPlayerData().metadata["isdead"] then
...
AddEventHandler('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            ZyoCore.Functions.Progressbar("robbing_player", "Robbing person..", math.random(5000, 7000), false, true, {
...
AddEventHandler('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(time) > 0 then
            ...
            ZyoCore.Functions.Notify("Tempo precisa de ser maior que 0..", "error")
        end
    else
        ZyoCore.Functions.Notify("Ninguem por perto!", "error")
    end
    ...
    if not IsPedRagdoll(GetPlayerPed(-1)) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(GetPlayerPed(-1))) then
                ...
                ZyoCore.Functions.Notify("Você não pode algemar alguém em um veículo", "error")
            end
        else
            ZyoCore.Functions.Notify("Ninguem por perto!", "error")
        end
        ...
        if player ~= -1 and distance < 1.5 then
            ZyoCore.Functions.TriggerCallback('ZyoCore:HasItem', function(result)
                if result then 
                    ...
                    ZyoCore.Functions.Notify("You don\'t have handcuffs on you", "error")
                end
            end, "handcuffs")
        else
            ZyoCore.Functions.Notify("Ninguem por perto!", "error")
...
AddEventHandler('police:client:GetCuffed', function(playerId, isSoftcuff)
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            ZyoCore.Functions.Notify("Você está algemado!")
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            ZyoCore.Functions.Notify("Você está algemado, mas pode andar")
        end
    else
        isHandcuffed = false
        isEscorted = false
        ...
        ClearPedTasksImmediately(GetPlayerPed(-1))
        ZyoCore.Functions.Notify("Você não está algemado!")
    end
end)
...
function IsTargetDead(playerId)
    local retval = false
    ZyoCore.Functions.TriggerCallback('police:server:isPlayerDead', function(result)
        retval = result
    end, playerId)
    Citizen.Wait(100)
    return retval
