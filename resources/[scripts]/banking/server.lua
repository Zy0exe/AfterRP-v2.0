...
AddEventHandler('bank:withdraw', function(amount)
    ...
    if bankamount >= amount and amount > 0 then
      ...
      TriggerClientEvent('ZyoCore:Notify', src, 'Não tens dinheiro suficiente no banco :(', 'error')
    end
end)
...
AddEventHandler('bank:deposit', function(amount)
    ...
    if cashamount >= amount and amount > 0 then
      ...
      TriggerClientEvent('ZyoCore:Notify', src, 'Não tens dinheiro suficiente para depositar :(', 'error')
    end
end)

ZyoCore.Commands.Add("dardinheiro", "Dê algum dinheiro a um jogador", {{name="id", help="Id do player"},{name="amount", help="Quantia de dinheiro"}}, true, function(source, args)
...
AddEventHandler('banking:server:giveCash', function(trgtId, amount)
  ...
  local Target = ZyoCore.Functions.GetPlayer(trgtId)
  ...
  if src ~= trgtId then
    ...
    TriggerClientEvent('ZyoCore:Notify', trgtId, "Você recebeu €"..amount.." from "..Player.PlayerData.charinfo.firstname.."!", 'success')
    TriggerClientEvent('ZyoCore:Notify', src, "Você deu €"..amount.." para "..Target.PlayerData.charinfo.firstname.."!", 'success')
  else
