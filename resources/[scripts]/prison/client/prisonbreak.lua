...
Citizen.CreateThread(function()
    ...
    requiredItems = {
        ...
        [2] = {name = ZyoCore.Shared.Items["gatecrack"]["name"], image = ZyoCore.Shared.Items["gatecrack"]["image"]},
    }
    while true do 
        Citizen.Wait(5)
        inRange = false
                ...
                for k, v in pairs(Gates) do
                    ...
                    if (dist < 1.5) then
                        ...
                        if securityLockdown then
                            ZyoCore.Functions.DrawText3D(Gates[k].coords.x, Gates[k].coords.y, Gates[k].coords.z, "~r~BLOQUEIO DE SISTEMA")
                        elseif Gates[k].hit then
                            ZyoCore.Functions.DrawText3D(Gates[k].coords.x, Gates[k].coords.y, Gates[k].coords.z, "BRECHA NO SISTEMA")
...
Citizen.CreateThread(function()
	while true do
		...
		if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["middle"].coords.x, Config.Locations["middle"].coords.y, Config.Locations["middle"].coords.z, false) > 200.0 and inJail) then
            ...
            ZyoCore.Functions.Notify("Você escapou .. Saia daqui!", "error")
		end
	end
end)
...
AddEventHandler('electronickit:UseElectronickit', function()
    if currentGate ~= 0 and not securityLockdown and not Gates[currentGate].hit then 
        ZyoCore.Functions.TriggerCallback('ZyoCore:HasItem', function(result)
            if result then 
                TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                ZyoCore.Functions.Progressbar("hack_gate", "A conectar o Kit Eletronico..", math.random(5000, 10000), false, true, {
                ...
                ZyoCore.Functions.Notify("Falta-te algo..", "error")
            end
        end, "gatecrack")
    end
end)
