...
AddEventHandler('prison:server:SetJailStatus', function(jailTime)
    ...
    if jailTime > 0 then
        if Player.PlayerData.job.name ~= "unemployed" then
            ...
            TriggerClientEvent('ZyoCore:Notify', src, "Estas desempregado..")
        end
    end
end)
...
ZyoCore.Functions.CreateUseableItem("electronickit", function(source, item)
...
ZyoCore.Functions.CreateCallback('prison:server:IsAlarmActive', function(source, cb)
    cb(AlarmActivated)
end)