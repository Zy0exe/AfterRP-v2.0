...
function takeHostage()
	...
	if closestPlayer ~= nil and closestPlayer ~= -1 then
		...
		for i=1, #hostageAllowedWeapons do
			if HasPedGotWeapon(GetPlayerPed(-1), GetHashKey(hostageAllowedWeapons[i]), false) then
				if GetAmmoInPedWeapon(GetPlayerPed(-1), GetHashKey(hostageAllowedWeapons[i])) > 0 then
					...
					ZyoCore.Functions.Notify("Não tens munição!")
					return			
				end	
			else
				NoWeapon = true	
		...
		if NoWeapon then
			ZyoCore.Functions.Notify("Não podes utilizar esta arma para fazer refens!")
			return
		end
		...
		ZyoCore.Functions.Notify("Ninguem por perto!")
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"[G] para libertar, [H] para matar")
