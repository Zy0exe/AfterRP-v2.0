...
ZyoCore.Commands.Add("darperms", "Conceda permissões a alguém (god/admin)", {{name="id", help="ID of player"}, {name="permission", help="Permission level"}}, true, function(source, args)
	local Player = ZyoCore.Functions.GetPlayer(tonumber(args[1]))
...
ZyoCore.Commands.Add("tirarperms", "Remover permissões de alguém", {{name="id", help="ID of player"}}, true, function(source, args)
	local Player = ZyoCore.Functions.GetPlayer(tonumber(args[1]))
	if Player ~= nil then
...
ZyoCore.Commands.Add("debug", "Ativar / desativar o modo de depuração", {}, false, function(source, args)
...
ZyoCore.Commands.Add("givemoney", "Dê dinheiro a um jogador", {{name="id", help="Player ID"},{name="moneytype", help="Type of money (cash, bank, crypto)"}, {name="amount", help="Amount of money"}}, true, function(source, args)
	local Player = ZyoCore.Functions.GetPlayer(tonumber(args[1]))
	if Player ~= nil then
...
ZyoCore.Commands.Add("trabalho", "Veja que trabalho você tem", {}, false, function(source, args)
...
ZyoCore.Commands.Add("gang", "Veja em qual gangue você está", {}, false, function(source, args)
	local Player = ZyoCore.Functions.GetPlayer(source)
	...
	if Player.PlayerData.gang.name ~= "geen" then
		...
		TriggerClientEvent('ZyoCore:Notify', source, "Você não está em uma gangue!", "error")
	end
end)
...
ZyoCore.Commands.Add("clearinv", "Limpe o inventário de um jogador", {{name="id", help="Player ID"}}, false, function(source, args)
