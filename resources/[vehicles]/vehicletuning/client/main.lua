...
function CalculateRepair()
	...
	if ZyoCore.Shared.VehicleModels[model] ~= nil then
		vehiclePrice = ZyoCore.Shared.VehicleModels[model]["price"]
	end
...
function DriveOutOfGarage(pos)
	...
	ZyoCore.Functions.Progressbar("vehicletune_editvehicle", "Bezig met Vehicle..", (editCount * 500), false, true, {
									...
									if button.costs ~= 0 then
										ZyoCore.Functions.GetPlayerData(function(PlayerData)
											...
											if cash >= button.costs or bank >= button.costs then
												...
												ZyoCore.Functions.Notify("Je hebt niet genoeg geld!", "error")
											end
										end)
									else
										ButtonSelected(button)
        ...
        if ShowEnginePos then
            ...
            if vehicle ~= 0 and vehicle ~= nil then
                ...
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0) and not IsPedInAnyVehicle(GetPlayerPed(-1)) then
                    ...
                    ZyoCore.Functions.DrawText3D(drawpos.x, drawpos.y, drawpos.z, "Sta hier..")
...
function RepairVehicle(vehicle)
	...
	ZyoCore.Functions.Progressbar("repair_vehicle", "Bezig met sleutelen..", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		ZyoCore.Functions.Notify("Vehicle gemaakt!")
		...
		StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
		ZyoCore.Functions.Notify("Mislukt!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
			SetVehicleDoorShut(vehicle, 5, false)
		else
			SetVehicleDoorShut(vehicle, 4, false)
