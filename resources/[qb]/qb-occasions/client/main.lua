            ...
            for _,slot in pairs(Config.OccasionSlots) do
                ...
                if dist <= 40 then
                    ...
                    if not vehiclesSpawned then
                        ...
                        ZyoCore.Functions.TriggerCallback('qb-occasions:server:getVehicles', function(vehicles)
                    ...
                    if dstCheck <= 2 then
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                ...
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, ZyoCore.Shared.Vehicles[Config.OccasionSlots[i]["model"]]["name"]..', Preço: €'..Config.OccasionSlots[i]["price"]..',-')
                                if Config.OccasionSlots[i]["owner"] == ZyoCore.Functions.GetPlayerData().citizenid then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    ZyoCore.Functions.TriggerCallback('qb-occasions:server:getSellerInformation', function(info)
                                        if info ~= nil then 
                        ...
                        if IsControlJustPressed(0, Keys["E"]) then
                            ...
                            ZyoCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
                                if owned then
                                    ...
                                    ZyoCore.Functions.Notify('Este veiculo pertence-te?', 'error', 3500)
                                end
                            end, VehiclePlate)
                        end
                    end
...
function spawnOccasionsVehicles(vehicles)
    ...
    if vehicles ~= nil then
        for i = 1, #vehicles, 1 do
            ...
            ZyoCore.Functions.SetVehicleProperties(oSlot[i]["occasionid"], json.decode(oSlot[i]["mods"]))
...
function despawnOccasionsVehicles()
    ...
    for i = 1, #Config.OccasionSlots, 1 do
        ...
        if oldVehicle ~= 0 then
            ZyoCore.Functions.DeleteVehicle(oldVehicle)
        end
    end
end
...
function openSellContract(bool)
    ...
    SendNUIMessage({
        action = "sellVehicle",
        pData = ZyoCore.Functions.GetPlayerData(),
...
RegisterNUICallback('error', function(data)
    ZyoCore.Functions.Notify(data.message, 'error')
end)
...
AddEventHandler('qb-occasions:client:BuyFinished', function(model, plate, mods)
    ...
    ZyoCore.Functions.SpawnVehicle(model, function(veh)
        ...
        ZyoCore.Functions.Notify("Veiculo comprado", "success", 2500)
        TriggerEvent("vehiclekeys:client:SetOwner", plate)
        SetVehicleEngineOn(veh, true, true)
        Citizen.Wait(500)
        ZyoCore.Functions.SetVehicleProperties(veh, vehmods)
...
AddEventHandler('qb-occasions:client:ReturnOwnedVehicle', function(vehdata)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(500)
    ZyoCore.Functions.SpawnVehicle(vehdata.model, function(veh)
        SetVehicleNumberPlateText(veh, vehdata.plate)
        SetEntityHeading(veh, Config.BuyVehicle.h)
        TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
        exports['LegacyFuel']:SetFuel(veh, 100)
        ZyoCore.Functions.Notify("Retiras-te o teu veiculo do stand ..")
...
RegisterNUICallback('sellVehicle', function(data)
    ...
    vehicleData.model = ZyoCore.Shared.VehicleModels[GetEntityModel(vehicleData.ent)]["model"]
    vehicleData.plate = GetVehicleNumberPlateText(GetVehiclePedIsUsing(GetPlayerPed(-1)))
    vehicleData.mods = ZyoCore.Functions.GetVehicleProperties(vehicleData.ent)
...
function sellVehicleWait(price)
    ...
    Citizen.Wait(1500)
    DoScreenFadeIn(250)
    ZyoCore.Functions.Notify('O teu carro esta a venda por €'..price..',-', 'success')
