...
ZyoCore.Functions.CreateCallback('qb-occasions:server:getVehicles', function(source, cb)
    ZyoCore.Functions.ExecuteSql(false, 'SELECT * FROM `occasion_vehicles`', function(result)
...
ZyoCore.Functions.CreateCallback("qb-garage:server:checkVehicleOwner", function(source, cb, plate)
    local src = source
...
ZyoCore.Functions.CreateCallback("qb-occasions:server:getSellerInformation", function(source, cb, citizenid)
    local src = source
...
AddEventHandler('qb-occasions:server:ReturnVehicle', function(vehicleData)
    ...
    ZyoCore.Functions.ExecuteSql(false, "SELECT * FROM `occasion_vehicles` WHERE `plate` = '"..vehicleData['plate'].."' AND `occasionid` = '"..vehicleData["oid"].."'", function(result)
        if result[1] ~= nil then 
            if result[1].seller == Player.PlayerData.citizenid then
                ZyoCore.Functions.ExecuteSql(false, "INSERT INTO `player_vehicles` (`steam`, `citizenid`, `vehicle`, `hash`, `mods`, `plate`, `state`) VALUES ('"..Player.PlayerData.steam.."', '"..Player.PlayerData.citizenid.."', '"..vehicleData["model"].."', '"..GetHashKey(vehicleData["model"]).."', '"..vehicleData["mods"].."', '"..vehicleData["plate"].."', '0')")
                ZyoCore.Functions.ExecuteSql(false, "DELETE FROM `occasion_vehicles` WHERE `occasionid` = '"..vehicleData["oid"].."' and `plate` = '"..vehicleData['plate'].."'")
                ...
                TriggerClientEvent('ZyoCore:Notify', src, 'Este não é o seu veículo...', 'error', 3500)
            end
        else
            TriggerClientEvent('ZyoCore:Notify', src, 'Veículo não existe...', 'error', 3500)
        end
    end)
end)
...
AddEventHandler('qb-occasions:server:sellVehicle', function(vehiclePrice, vehicleData)
    local src = source
    local Player = ZyoCore.Functions.GetPlayer(src)
    ZyoCore.Functions.ExecuteSql(true, "DELETE FROM `player_vehicles` WHERE `plate` = '"..vehicleData.plate.."' AND `vehicle` = '"..vehicleData.model.."'")
    ZyoCore.Functions.ExecuteSql(true, "INSERT INTO `occasion_vehicles` (`seller`, `price`, `description`, `plate`, `model`, `mods`, `occasionid`) VALUES ('"..Player.PlayerData.citizenid.."', '"..vehiclePrice.."', '"..escapeSqli(vehicleData.desc).."', '"..vehicleData.plate.."', '"..vehicleData.model.."', '"..json.encode(vehicleData.mods).."', '"..generateOID().."')")
    ...
    ZyoCore.Functions.ExecuteSql(false, "SELECT * FROM `occasion_vehicles` WHERE `plate` = '"..vehicleData['plate'].."' AND `occasionid` = '"..vehicleData["oid"].."'", function(result)
        if result[1] ~= nil and next(result[1]) ~= nil then
            if Player.PlayerData.money.cash >= result[1].price then
                ...
                local SellerData = ZyoCore.Functions.GetPlayerByCitizenId(SellerCitizenId)
                ...
                ZyoCore.Functions.ExecuteSql(false, "INSERT INTO `player_vehicles` (`steam`, `citizenid`, `vehicle`, `hash`, `mods`, `plate`, `state`) VALUES ('"..Player.PlayerData.steam.."', '"..Player.PlayerData.citizenid.."', '"..result[1].model.."', '"..GetHashKey(result[1].model).."', '"..result[1].mods.."', '"..result[1].plate.."', '0')")
                ...
                if SellerData ~= nil then
                    ...
                    ZyoCore.Functions.ExecuteSql(true, "SELECT * FROM `players` WHERE `citizenid` = '"..SellerCitizenId.."'", function(BuyerData)
                        if BuyerData[1] ~= nil then
                            local BuyerMoney = json.decode(BuyerData[1].money)
                            BuyerMoney.bank = BuyerMoney.bank + NewPrice
                            ZyoCore.Functions.ExecuteSql(false, "UPDATE `players` SET `money` = '"..json.encode(BuyerMoney).."' WHERE `citizenid` = '"..SellerCitizenId.."'")
                        end
                    end)
                end
                ...
                ZyoCore.Functions.ExecuteSql(false, "DELETE FROM `occasion_vehicles` WHERE `plate` = '"..result[1].plate.."' and `occasionid` = '"..result[1].occasionid.."'")
                ...
                TriggerEvent('qb-phone:server:sendNewMailToOffline', SellerCitizenId, {
                    ...
                    message = "Olá sou o Jorge vim cá dizer que seu carro foi vendido por "..ZyoCore.Shared.Vehicles[result[1].model].name.."€"..result[1].price.."!"
                })
            elseif Player.PlayerData.money.bank >= result[1].price then
                ...
                TriggerEvent('qb-phone:server:sendNewMailToOffline', SellerCitizenId, {
                    ...
                    message = "Olá teu carro "..ZyoCore.Shared.Vehicles[result[1].model].name.." foi vendido por €"..result[1].price..",-!"
                })
            else
                TriggerClientEvent('ZyoCore:Notify', src, 'Não tens dinheiro suficiente...', 'error', 3500)
            end
        end
    end)
end)
