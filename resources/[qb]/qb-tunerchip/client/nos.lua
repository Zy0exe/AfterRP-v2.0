...
AddEventHandler('ZyoCore:Client:OnPlayerLoaded', function()
    ZyoCore.Functions.TriggerCallback('nitrous:GetNosLoadedVehs', function(vehs)
        VehicleNitrous = vehs
    end)
end)
...
AddEventHandler('smallresource:client:LoadNitrous', function()
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            ZyoCore.Functions.Progressbar("use_nos", "A meter nitro...", 1000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
            }, {}, {}, {}, function() -- Done
                TriggerEvent("inventory:client:ItemBox", ZyoCore.Shared.Items['nitrous'], "remove")
                TriggerServerEvent("ZyoCore:Server:RemoveItem", 'nitrous', 1)
                local CurrentVehicle = GetVehiclePedIsIn(GetPlayerPed(-1))
                local Plate = GetVehicleNumberPlateText(CurrentVehicle)
                TriggerServerEvent('nitrous:server:LoadNitrous', Plate)
            end)
        else
            ZyoCore.Functions.Notify('Não estas dentro de um carro.', 'error')
        end
    else
        ZyoCore.Functions.Notify('Já tens equipado o nitro..', 'error')
    end
end)
