...
RegisterNUICallback('save', function(data)
    ZyoCore.Functions.TriggerCallback('qb-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = GetPlayerPed(-1)
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            ZyoCore.Functions.Notify('TunerJN v1.05: mudança de veículo!', 'error')
...
AddEventHandler('qb-tunerchip:server:TuneStatus', function()
    ...
    ZyoCore.Functions.TriggerCallback('qb-tunerchip:server:GetStatus', function(status)
        if status then
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    ZyoCore.Functions.Notify('TunerJN v1.05: Veículo reiniciado!', 'error')
end)
...
AddEventHandler('qb-tunerchip:client:openChip', function()
    ...
    if inVehicle then
        ZyoCore.Functions.Progressbar("connect_laptop", "TunerJN está conectando..", 2000, false, true, {
            ...
            StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
            ZyoCore.Functions.Notify("Geannuleerd..", "error")
        end)
    else
        ZyoCore.Functions.Notify("Você não está em um veículo..", "error")
    end
end)
