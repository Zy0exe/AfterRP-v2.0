                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                ZyoCore.Functions.TriggerCallback('qb-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        ZyoCore.Functions.Notify("Recebeste de volta os teus 1000€!")
                                    else
                                        ZyoCore.Functions.Notify("Não pagaste nenhuma caução neste veiculo..")
                                    end
                                end)
                            end
                        else
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                ZyoCore.Functions.TriggerCallback('qb-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        local coords = Config.Locations["vehicle"].coords
                                        ZyoCore.Functions.SpawnVehicle("trash2", function(veh)
                                            GarbageVehicle = veh
                                            ...
                                            IsWorking = true
                                            SetGarbageRoute()
                                            ZyoCore.Functions.Notify("Pagaste 1000€ como caução do veiculo!")
                                            ZyoCore.Functions.Notify("Começaste a tua rota, foi definida uma localização no teu GPS!")
                                        end, coords, true)
                                    else
                                        ZyoCore.Functions.Notify("Não tens dinheiro suficiente para a caução... A caução custa "..Config.BailPrice)
                                    end
                                end)
                            end
                        end
                                            ...
                                            if IsControlJustPressed(0, 51) then
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        SetGarbageRoute()
                                                        ZyoCore.Functions.Notify("O contentor foi limpo, avança para a proxima localização!")
                                                    else
                                                        -- Hier ben je klaar met werken.
                                                        ZyoCore.Functions.Notify("Acabaste o turno, volta para a tua central.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        ZyoCore.Functions.Notify("Ainda faltam "..AmountOfBags.." sacos!")
                                                    else
                                                        ZyoCore.Functions.Notify("Ainda falta "..AmountOfBags.." saco!")
                                                    end
                                                    hasZak = false
                                                end
                                                DeliverAnim()
