...
AddEventHandler('qb-weed:client:getHousePlants', function(house)    
    ZyoCore.Functions.TriggerCallback('qb-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
                ...
                for k, v in pairs(housePlants[currentHouse]) do
                    ...
                    if plyDistance < 0.8 then
                        ...
                        if plantData["plantStats"]["health"] > 0 then
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                ...
                                if IsControlJustPressed(0, QBWeed.Keys["E"]) then
                                    ZyoCore.Functions.Progressbar("remove_weed_plant", "A iniciar o cultivo", 8000, false, true, {
                        ...
                        elseif plantData["plantStats"]["health"] == 0 then
                            ...
                            if IsControlJustPressed(0, QBWeed.Keys["E"]) then
                                ZyoCore.Functions.Progressbar("remove_weed_plant", "Remover planta", 8000, false, true, {
                                    disableMovement = true,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true,
...
AddEventHandler('qb-weed:client:placePlant', function(type, item)
    ...
    if currentHouse ~= nil then
        if ClosestPlant == 0 then
            ZyoCore.Functions.Progressbar("plant_weed_plant", "Planting the plant", 8000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
            ...
            ZyoCore.Functions.Notify('Não tens espaço para a planta.', 'error', 3500)
        end
    else
        ZyoCore.Functions.Notify("Não é seguro fazeres isso aqui.", 'error', 3500)
    end
end)
        ...
        if ClosestTarget ~= 0 then
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    ZyoCore.Functions.Notify('A planta não precisa de ser regada.', 'error', 3500)
                else
                    ZyoCore.Functions.Progressbar("plant_weed_plant", "'A regar a planta.", math.random(4000, 8000), false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                ...
                ZyoCore.Functions.Notify("Sem espaço para a planta.", "error")
            end
        else
            ZyoCore.Functions.Notify("Sem espaço para a planta.", "error")
        end
