TryToFish = function()
    ZyoCore.Functions.TriggerCallback('qb-fishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then return ZyoCore.Functions.Notify("Você não pode nadar e pescar ao mesmo tempo.", "error") end 
        if IsPedInAnyVehicle(cachedData["ped"]) then return ZyoCore.Functions.Notify("Você precisa sair do veículo para começar a pescar.", "error") end 
        if count ~= nil then
            if count == 0 then
                ZyoCore.Functions.Notify("Você precisa de uma vara de pescar e isca para pescar.", "primary")
            else
                ...
                if waterValidated then
                    ...
                    ZyoCore.Functions.Notify("Você precisa mirar na água para pescar", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        ZyoCore.Functions.Notify('Você não tem nenhuma isca!', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        if GetGameTimer() - startedCasting > 5000 then
            ZyoCore.Functions.Notify("Você precisa lançar a isca.", "primary")
    ...
    ZyoCore.Functions.Notify("Esperando um peixe morder...", "success", "10000")
    TriggerServerEvent('ZyoCore:Server:RemoveItem', "fishingbait", 1)
	TriggerEvent("inventory:client:ItemBox", ZyoCore.Shared.Items["fishingbait"], "remove")
    ...
    if caughtFish then
        ...
        ZyoCore.Functions.Notify("O peixe se soltou.", "error")
    end
...
IsInWater = function()
    ...
    ZyoCore.Functions.Notify("Verificando o local de pesca...", "success", "3000")
...
function HasFishingBait()
    local rtval = false
    ZyoCore.Functions.TriggerCallback('ZyoCore:HasItem', function(amount)
        if amount then
            rtval = true
        end
    end, "fishingbait")
