...
ZyoCore.Functions.CreateCallback('qb-fishing:GetItemData', function(source, cb, itemName)
	local retval = false
...
ZyoCore.Functions.CreateUseableItem("fishingrod", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
...
AddEventHandler('qb-fishing:receiveFish', function(cabin, house)
    ...
    if itemFound then
        for i = 1, itemCount, 1 do
            ...
            local itemInfo = ZyoCore.Shared.Items[randomItem]
            if luck == 100 then
                randomItem = "killerwhalemeat"
                itemInfo = ZyoCore.Shared.Items[randomItem]
...
AddEventHandler("qb-fishing:sellFish", function()
    ...
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
		...
		TriggerClientEvent('ZyoCore:Notify', src, "Voce vendeu seu peixe")
	end
end)