...
AddEventHandler('qb-trucker:server:DoBail', function(bool, vehInfo)
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('ZyoCore:Notify', src, 'You paid the deposit of 1000,-  (Cash)', 'success')
        ...
        elseif Player.PlayerData.money.bank >= Config.BailPrice then
            ...
            TriggerClientEvent('ZyoCore:Notify', src, 'You paid the deposit of 1000,- (Bank)', 'success')
            TriggerClientEvent('qb-trucker:client:SpawnVehicle', src, vehInfo)
        else
            TriggerClientEvent('ZyoCore:Notify', src, 'You dont have enough cash you need 1000,-', 'error')
        end
    else
        if Bail[Player.PlayerData.citizenid] ~= nil then
            Player.Functions.AddMoney('cash', Bail[Player.PlayerData.citizenid], "trucker-bail-paid")
            Bail[Player.PlayerData.citizenid] = nil
            TriggerClientEvent('ZyoCore:Notify', src, 'You recived you deposit of 1000,-', 'success')
        end
    end
end)
