...
Citizen.CreateThread(function()
    ZyoCore.Functions.ExecuteSql(false, "SELECT * FROM `moneysafes`", function(safes)
        if safes[1] ~= nil then
...
ZyoCore.Commands.Add("depositar", "Depositar dinheiro na empresa", {}, false, function(source, args)
...
ZyoCore.Commands.Add("tirar", "Tirar dinheiro da empresa", {}, false, function(source, args)
    local Player = ZyoCore.Functions.GetPlayer(source)
    local amount = tonumber(args[1]) or 0
...
AddEventHandler('qb-moneysafe:server:DepositMoney', function(safe, amount, sender)
    ...
    elseif Player.PlayerData.money.bank >= amount then
        ...
        TriggerClientEvent('ZyoCore:Notify', src, "Não tens dinheiro", "error")
        return
    end
    if sender == nil then
    ...
    ZyoCore.Functions.ExecuteSql(false, "SELECT * FROM `moneysafes` WHERE `safe` = '"..safe.."'", function(result)
        if result[1] ~= nil then
            Config.Safes[safe].money = (Config.Safes[safe].money + amount)
            ZyoCore.Functions.ExecuteSql(false, "UPDATE `moneysafes` SET money = '"..Config.Safes[safe].money.."', transactions = '"..json.encode(Config.Safes[safe].transactions).."' WHERE `safe` = '"..safe.."'")
        else
            Config.Safes[safe].money = amount
            ZyoCore.Functions.ExecuteSql(false, "INSERT INTO `moneysafes` (`safe`, `money`, `transactions`) VALUES ('"..safe.."', '"..Config.Safes[safe].money.."', '"..json.encode(Config.Safes[safe].transactions).."')")
        end
        TriggerClientEvent('qb-moneysafe:client:UpdateSafe', -1, Config.Safes[safe], safe)
        TriggerClientEvent('ZyoCore:Notify', src, "Meteste €"..amount..",- no cofre da empresa", "success")
    end)
end)
...
AddEventHandler('qb-moneysafe:server:WithdrawMoney', function(safe, amount)
    ...
    if (Config.Safes[safe].money - amount) >= 0 then 
        ...
        ZyoCore.Functions.ExecuteSql(false, "UPDATE `moneysafes` SET money = '"..Config.Safes[safe].money.."', transactions = '"..json.encode(Config.Safes[safe].transactions).."' WHERE `safe` = '"..safe.."'")
        TriggerClientEvent('qb-moneysafe:client:UpdateSafe', -1, Config.Safes[safe], safe)
        TriggerClientEvent('ZyoCore:Notify', src, "Pegaste €"..amount..",- fora do cofre", "success")
        Player.Functions.AddMoney('cash', amount)
    else
        TriggerClientEvent('ZyoCore:Notify', src, "Não há dinheiro suficiente no cofre.", "error")
    end
end)