...
ZyoCore.Commands.Add("sneeuwbal", "Picking up snowball", {}, false, function(source, args)
    if CurrentWeather == "XMAS" then
        TriggerClientEvent("inventory:client:PickupSnowballs", source)
    else
        TriggerClientEvent('ZyoCore:Notify', source, "Not the right time of the year :(", "error")
    end
end)