...
ZyoCore.Functions.CreateCallback("weapons:server:GetConfig", function(source, cb)
...
--     ZyoCore.Functions.ExecuteSql(false, "SELECT * FROM `playerammo` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
...
ZyoCore.Functions.CreateCallback("weapon:server:GetWeaponAmmo", function(source, cb, WeaponData)
...
ZyoCore.Functions.CreateUseableItem("pistol_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_PISTOL", 12, item)
end)

ZyoCore.Functions.CreateUseableItem("sniper_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "SNIPER_PISTOL", 5, item)
end)

ZyoCore.Functions.CreateUseableItem("rifle_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_RIFLE", 30, item)
end)

ZyoCore.Functions.CreateUseableItem("smg_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_SMG", 20, item)
end)

ZyoCore.Functions.CreateUseableItem("shotgun_ammo", function(source, item)
    TriggerClientEvent("weapon:client:AddAmmo", source, "AMMO_SHOTGUN", 10, item)
end)

ZyoCore.Functions.CreateUseableItem("mg_ammo", function(source, item)
...
--     local src = source
--     local Player = ZyoCore.Functions.GetPlayer(src)
--     local WeaponData = ZyoCore.Shared.Weapons[GetHashKey(data.name)]
...
--                 TriggerClientEvent('ZyoCore:Notify', src, "Your weapon is broken, u need to repair it before u can use it again.", "error")
--             end
--         else
            ...
            if WeaponSlot.info.quality ~= nil then
                for i = 1, RepeatAmount, 1 do
                    if WeaponSlot.info.quality - DecreaseAmount > 0 then
                        ...
                        WeaponSlot.info.quality = 0
                        TriggerClientEvent('inventory:client:UseWeapon', src, data)
                        TriggerClientEvent('ZyoCore:Notify', src, "Sua arma está quebrada, você precisa consertá-la antes de usá-la novamente.", "error")
                        break
                    end
                end
            else
...
ZyoCore.Commands.Add("repairweapon", "Comando de reparo de arma para equipe", {{name="hp", help="HP da sua arma"}}, true, function(source, args)
...
ZyoCore.Functions.CreateCallback("weapons:server:RepairWeapon", function(source, cb, RepairPoint, data)
    local src = source
    local Player = ZyoCore.Functions.GetPlayer(src)
    ...
    if Player.PlayerData.items[data.slot] ~= nil then
        if Player.PlayerData.items[data.slot].info.quality ~= nil then
            if Player.PlayerData.items[data.slot].info.quality ~= 100 then
                if Player.Functions.RemoveMoney('cash', Config.WeaponRepairCotsts[WeaponClass]) then
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items[data.name], "remove")
                ...
                TriggerClientEvent("ZyoCore:Notify", src, "Esta arma não está danificada..", "error")
                cb(false)
            end
        else
            TriggerClientEvent("ZyoCore:Notify", src, "Esta arma não está danificada..", "error")
        ...
        TriggerClientEvent('ZyoCore:Notify', src, "Você não tinha uma arma em suas mãos..", "error")
...
AddEventHandler("weapons:server:TakeBackWeapon", function(k, data)
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items[itemdata.name], "add")
    Config.WeaponRepairPoints[k].IsRepairing = false
    Config.WeaponRepairPoints[k].RepairingData = {}
    TriggerClientEvent('weapons:client:SyncRepairShops', -1, Config.WeaponRepairPoints[k], k)
end)

ZyoCore.Functions.CreateUseableItem("pistol_suppressor", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

ZyoCore.Functions.CreateUseableItem("smg_suppressor", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

ZyoCore.Functions.CreateUseableItem("rifle_suppressor", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "suppressor")
end)

ZyoCore.Functions.CreateUseableItem("pistol_extendedclip", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

ZyoCore.Functions.CreateUseableItem("smg_extendedclip", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

ZyoCore.Functions.CreateUseableItem("smg_flashlight", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "flashlight")
end)

ZyoCore.Functions.CreateUseableItem("smg_scope", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
...
ZyoCore.Functions.CreateUseableItem("rifle_extendedclip", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
    TriggerClientEvent("weapons:client:EquipAttachment", source, item, "extendedclip")
end)

ZyoCore.Functions.CreateUseableItem("rifle_drummag", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
    ...
    if Inventory[CurrentWeaponData.slot] ~= nil then
        if Inventory[CurrentWeaponData.slot].info.attachments ~= nil and next(Inventory[CurrentWeaponData.slot].info.attachments) ~= nil then
            ...
            if not HasAttach then
                ...
                TriggerClientEvent("ZyoCore:Notify", src, "Você já tem "..AttachmentData.label:lower().." na sua arma.", "error", 3500)
            end
        else
...
ZyoCore.Functions.CreateCallback('weapons:server:RemoveAttachment', function(source, cb, AttachmentData, ItemData)
    local src = source
    local Player = ZyoCore.Functions.GetPlayer(src)
    local Inventory = Player.PlayerData.items
    ...
    if Inventory[ItemData.slot] ~= nil then
        if Inventory[ItemData.slot].info.attachments ~= nil and next(Inventory[ItemData.slot].info.attachments) ~= nil then
            ...
            if HasAttach then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items[AttachmentComponent.item], "add")
                TriggerClientEvent("ZyoCore:Notify", src, "Você removeu "..AttachmentComponent.label.." da sua arma!", "error")
