...
RegisterNetEvent('ZyoCore:Client:OnJobUpdate')
AddEventHandler('ZyoCore:Client:OnJobUpdate', function(JobInfo)
    SendNUIMessage({
        ...
        if IsControlJustPressed(0, Config.OpenPhone) then
            if not PhoneData.isOpen then
                ...
                if not IsHandcuffed then
                    ...
                    ZyoCore.Functions.Notify("Não podes fazer isso neste momento", "error")
                end
            end
        end
        Citizen.Wait(3)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetPhoneData', function(pData)   
...
function LoadPhone()
    ...
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetPhoneData', function(pData)
        ...
        PhoneData.PlayerData = ZyoCore.Functions.GetPlayerData()
...
RegisterNUICallback('HasPhone', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:HasPhone', function(HasPhone)
        cb(HasPhone)
    end)
end)
...
function OpenPhone()
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetGarageVehicles', function(vehicles)
                PhoneData.GarageVehicles = vehicles
            end)
        else
            ZyoCore.Functions.Notify("Não tens um telemóvel", "error")
        end
    end)
end
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
    ...
    if New then
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Whatsapp",
                        text = "Mensagem nova de "..IsNumberInContacts(SenderNumber).."!",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 1500,
                    },
                ...
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Whatsapp",
                        text = "Porquê que estás a enviar mensagens a ti mesmo?!",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 4000,
                    },
            ...
            Wait(100)
            ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetContactPictures', function(Chats)
                SendNUIMessage({
                    action = "UpdateChat",
            ...
            SendNUIMessage({
                ...
                NotifyData = {
                    title = "Whatsapp",
                    content = "Recebeste uma mensagem de "..IsNumberInContacts(SenderNumber).."!",
                    icon = "fab fa-whatsapp",
                    timeout = 3500,
                    color = "#25D366",
                },
        ...
        if PhoneData.isOpen then
            ...
            SendNUIMessage({
                ...
                NotifyData = {
                    title = "Whatsapp",
                    content = "Recebeste uma mensagem nova de "..IsNumberInContacts(SenderNumber).."!",
                    icon = "fab fa-whatsapp",
                    timeout = 3500,
                    color = "#25D366",
                },
...
AddEventHandler('qb-phone_new:client:NewMailNotify', function(MailData)
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Mail",
                text = "Recebeste um mail novo de "..MailData.sender,
        ...
        SendNUIMessage({
            ...
            NotifyData = {
                title = "Mail",
                content = "Recebeste um mail novo de "..MailData.sender,
                icon = "fas fa-envelope",
                timeout = 3500,
                color = "#ff002f",
            },
...
AddEventHandler('qb-phone_new:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Anúncios",
                text = "Foi publicado um anúncio novo por "..LastAd,
                icon = "fas fa-ad",
                color = "#ff8f1a",
                timeout = 2500,
            },
        ...
        SendNUIMessage({
            ...
            NotifyData = {
                title = "Anúncios",
                content = "Foi publicado um anúncio novo por "..LastAd,
                icon = "fas fa-ad",
                timeout = 2500,
                color = "#ff8f1a",
            },
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:PayInvoice', function(CanPay, Invoices)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:DeclineInvoice', function(CanPay, Invoices)
                ...
                if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                    ...
                    SetTimeout(2500, function()
                        SendNUIMessage({
                            ...
                            PhoneNotify = {
                                title = "Twitter",
                                text = "Não te podes mencionar a ti mesmo!",
                                icon = "fab fa-twitter",
                                color = "#1DA1F2",
                            },
                        })
...
AddEventHandler('qb-phone_new:client:TransferMoney', function(amount, newmoney)
    ...
    if PhoneData.isOpen then
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Banco", text = "Tens &euro;"..amount.." disponíveis!", icon = "fas fa-university", color = "#8c7ae6", }, })
        SendNUIMessage({ action = "UpdateBank", NewBalance = PhoneData.PlayerData.money.bank })
    else
        SendNUIMessage({ action = "Notification", NotifyData = { title = "Banco", content = "Tens &euro;"..amount.." disponíveis!", icon = "fas fa-university", timeout = 2500, color = nil, }, })
    end
end)
    ...
    if src ~= MyPlayerId then
        if not PhoneData.isOpen then
            SendNUIMessage({
                action = "Notification",
                NotifyData = {
                    title = "Tweet Novo (@"..NewTweetData.firstName.." "..NewTweetData.lastName..")",
...
AddEventHandler('qb-phone_new:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    if not PhoneData.isOpen then
        SendNUIMessage({ action = "Notification", NotifyData = { title = "Foste mencionado num tweet!", content = TweetMessage.message, icon = "fab fa-twitter", timeout = 3500, color = nil, }, })
    else
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Foste mencionado num tweet!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#1DA1F2", }, })
    end
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    local PlayerData = ZyoCore.Functions.GetPlayerData()

    if (PlayerData.money.bank - amount) >= 0 then
        ZyoCore.Functions.TriggerCallback('qb-phone_new:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('CallContact', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetCallState', function(CanCall, IsOnline)
        local status = { 
            CanCall = CanCall, 
            IsOnline = IsOnline,
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({ 
            ...
            PhoneNotify = { 
                title = "Telemóvel",
                text = "Não tens nenhuma chamada por atender...",
                icon = "fas fa-phone",
                color = "#e84118",
            }, 
        })
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)

RegisterNUICallback('FetchVehicleResults', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetVehicleSearchResults', function(result)
        if result ~= nil then 
            for k, v in pairs(result) do
                ZyoCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    ...
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:ScanPlate', function(result)
        ZyoCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
            local vehicleInfo = ZyoCore.Shared.Vehicles[ZyoCore.Shared.VehicleModels[model]["model"]] ~= nil and ZyoCore.Shared.Vehicles[ZyoCore.Shared.VehicleModels[model]["model"]] or {["brand"] = "Unknown brand..", ["name"] = ""}
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    ZyoCore.Functions.Notify('Localização marcada no GPS para: '..data.alert.title)
...
function GetClosestPlayer()
    local closestPlayers = ZyoCore.Functions.GetPlayersFromCoords()
...
AddEventHandler('qb-phone_new:client:GiveContactDetails', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        ZyoCore.Functions.Notify("Ninguém por perto!", "error")
    end
end)
...
AddEventHandler('qb-phone_new:client:AddNewSuggestion', function(SuggestionData)
    ...
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Telemóvel",
                text = "Tens um contacto sugerido novo!",
                icon = "fa fa-phone-alt",
                color = "#04b543",
                timeout = 1500,
            },
        ...
        SendNUIMessage({
            ...
            NotifyData = {
                title = "Telemóvel",
                content = "Tens um contacto sugerido novo!",
                icon = "fa fa-phone-alt",
                timeout = 3500,
                color = "#04b543",
            },
...
RegisterNUICallback('GetCryptoData', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)
...
AddEventHandler('qb-phone_new:client:RemoveBankMoney', function(amount)
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Banco",
                text = "Foram debitados €"..amount..",- da tua conta!",
                icon = "fas fa-university",
                color = "#ff002f",
                timeout = 3500,
            },
        ...
        SendNUIMessage({
            ...
            NotifyData = {
                title = "Banco",
                content = "Foram debitados €"..amount..",- da tua conta!",
                icon = "fas fa-university",
                timeout = 3500,
                color = "#ff002f",
            },
...
RegisterNUICallback('GetAvailableRaces', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)
...
RegisterNUICallback('GetRaces', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('HasCreatedRace', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)
...
RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        local data = {
...
RegisterNUICallback('GetRacingLeaderboards', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        local ped = GetPlayerPed(-1)
        ...
        if dist <= 115.0 then
            ...
            ZyoCore.Functions.Notify('Estás muito longe da corrida. A localização foi marcada no teu GPS.', 'error', 5000)
...
RegisterNUICallback('CanRaceSetup', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)

RegisterNUICallback('GetPlayerKeys', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    ZyoCore.Functions.Notify("O teu GPS foi marcado para " .. data.HouseData.HouseData.adress .. "!", "success")
end)
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
    end, TransferedCid, data.HouseData)
end)

RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    ZyoCore.Functions.Notify('GPS marcado!', 'success')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)

RegisterNUICallback('GetCurrentTaxis', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetCurrentTaxis', function(taxis)
        cb(taxis)
    end)
end)

RegisterNUICallback('GetCurrentMecanicos', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-phone_new:server:GetCurrentMecanicos', function(mecanicos)
        cb(mecanicos)
    end)
end)
...
RegisterNUICallback('GetTruckerData', function(data, cb)
    local TruckerMeta = ZyoCore.Functions.GetPlayerData().metadata["jobrep"]["trucker"]
