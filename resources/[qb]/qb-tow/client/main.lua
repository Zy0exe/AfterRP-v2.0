...
AddEventHandler('jobs:client:ToggleNpc', function()
    if ZyoCore.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then 
            ZyoCore.Functions.Notify("Termina o teu trabalho primeiro!", "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
AddEventHandler('qb-tow:client:TowVehicle', function()
    ...
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then 
            --[[ Replaced "ZyoCore.Functions.GetClosestVehicle()" with custom implementation "getVehicleInDirection"
                 ZyoCore native could not return polcice and other vehicles types (NPC) ]] 
            ...
            if NpcOn and CurrentLocation ~= nil then
                if GetEntityModel(targetVehicle) ~= GetHashKey(CurrentLocation.model) then
                    ZyoCore.Functions.Notify("Não podes usar este veículo..", "error")
                    return
                end
            end
            if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
                if vehicle ~= targetVehicle then
                    ...
                    if GetDistanceBetweenCoords(towPos.x, towPos.y, towPos.z, targetPos.x, targetPos.y, targetPos.z, true) < 11.0 then
                        ZyoCore.Functions.Progressbar("towing_vehicle", "A rebocar o veículo..", 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                ZyoCore.Functions.Notify("Leva o veículo para Garagem de Rebocados!", "success", 5000)
                            end
                            ZyoCore.Functions.Notify("Veículo rebocado!")
                        end, function() -- Cancel
                            StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_ped", 1.0)
                            ZyoCore.Functions.Notify("Falhaste!", "error")
                        end)
                    end
                end
            end
        else
            ZyoCore.Functions.Progressbar("untowing_vehicle", "A retirar o veículo...", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                ZyoCore.Functions.Notify("Veículo retirado!")
            end, function() -- Cancel
                StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_ped", 1.0)
                ZyoCore.Functions.Notify("Falhaste!", "error")
            end)
        ...
        ZyoCore.Functions.Notify("Deves estar num reboque primeiro..", "error")
    end
end)
...
Citizen.CreateThread(function()
    ...
    while true do 
        Citizen.Wait(1)
        if isLoggedIn and ZyoCore ~= nil then
            if PlayerJob.name == "tow" then
                    ...
                    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 1.5) then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            if JobsDone > 0 then
                                ...
                                ZyoCore.Functions.Notify("Ainda não fizeste nenhum trabalho..", "error")
                            end
                        end
                ...
                if NpcOn and CurrentLocation ~= nil and next(CurrentLocation) ~= nil then
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, true) < 50.0 and not VehicleSpawned then
                        VehicleSpawned = true
                        ZyoCore.Functions.SpawnVehicle(CurrentLocation.model, function(veh)
...
function deliverVehicle(vehicle)
    ...
    JobsDone = JobsDone + 1
    VehicleSpawned = false
    ZyoCore.Functions.Notify("Entregaste o veículo!", "success")
    ZyoCore.Functions.Notify("Podes retirar o veículo")
