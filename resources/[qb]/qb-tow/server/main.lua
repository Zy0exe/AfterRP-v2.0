...
AddEventHandler('qb-tow:server:DoBail', function(bool, vehInfo)
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('ZyoCore:Notify', src, 'Você pagou seu depósito de 1000,', 'success')
        ...
        elseif Player.PlayerData.money.bank >= Config.BailPrice then
            ...
            TriggerClientEvent('ZyoCore:Notify', src, 'Você não tem dinheiro suficiente, o depósito é de 1000,', 'error')
        end
    else
        if Bail[Player.PlayerData.citizenid] ~= nil then
            Player.Functions.AddMoney('cash', Bail[Player.PlayerData.citizenid], "tow-bail-paid")
            Bail[Player.PlayerData.citizenid] = nil
            TriggerClientEvent('ZyoCore:Notify', src, 'Você recebeu seu depósito de 1000, de volta', 'success')
        end
    end
end)
...
ZyoCore.Commands.Add("npc", "Alternar opção de trabalho npc", {}, false, function(source, args)
	TriggerClientEvent("jobs:client:ToggleNpc", source)
end)

ZyoCore.Commands.Add("tow", "Coloque o carro na parte traseira do caminhão de reboque", {}, false, function(source, args)
