    ...
    while true do
        ...
        elseif WarMenu.IsMenuOpened('permissionOptions') then
            if WarMenu.ComboBox('Permission Group', perms, currentPermIndex, selectedPermIndex, function(currentIndex, selectedIndex)
                ...
                ZyoCore.Functions.Notify('You changed '..GetPlayerName(currentPlayer)..'\'s group to '..group.label)
            end
            WarMenu.Display()
        ...
        elseif WarMenu.IsMenuOpened('weatherOptions') then
            for k, v in pairs(AvailableWeatherTypes) do
                if WarMenu.MenuButton(AvailableWeatherTypes[k].label, 'weatherOptions') then
                    ...
                    ZyoCore.Functions.Notify('Weather changed to: '..AvailableWeatherTypes[k].label)
                end
            end
        ...
        elseif WarMenu.IsMenuOpened('createDealer') then
            if WarMenu.ComboBox('Hora minima', times, currentMinTimeIndex, selectedMinTimeIndex, function(currentIndex, selectedIndex)
                ...
                ZyoCore.Functions.Notify('Hora confirmada!', 'success')
            end
...
AddEventHandler('qb-admin:client:SaveCar', function()
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        local props = ZyoCore.Functions.GetVehicleProperties(veh)
        local hash = props.model
        if ZyoCore.Shared.VehicleModels[hash] ~= nil and next(ZyoCore.Shared.VehicleModels[hash]) ~= nil then
            TriggerServerEvent('qb-admin:server:SaveCar', props, ZyoCore.Shared.VehicleModels[hash], GetHashKey(veh), plate)
        else
            ZyoCore.Functions.Notify('You cant store this vehicle in your garage..', 'error')
        end
    else
        ZyoCore.Functions.Notify('You are not in a vehicle..', 'error')
    end
end)
...
AddEventHandler('qb-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    ...
    if weapon ~= "current" then
        ...
        ZyoCore.Functions.Notify('+'..ammo..' Ammo for the '..ZyoCore.Shared.Weapons[GetHashKey(weapon)]["label"], 'success')
    else
        local weapon = GetSelectedPedWeapon(ped)
        if weapon ~= nil then
            SetPedAmmo(ped, weapon, ammo)
            ZyoCore.Functions.Notify('+'..ammo..' Ammo for the '..ZyoCore.Shared.Weapons[weapon]["label"], 'success')
        else
            ZyoCore.Functions.Notify('You dont have a weapon in your hands..', 'error')
        end
    end
end)
