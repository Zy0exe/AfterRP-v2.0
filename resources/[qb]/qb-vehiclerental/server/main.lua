...
AddEventHandler('qb-vehiclerental:server:SetVehicleRented', function(plate, bool, vehicleData)
    ...
    if bool then
        if ply.PlayerData.money.cash >= vehicleData.price then
            ...
            TriggerClientEvent('ZyoCore:Notify', src, 'Je hebt de borg van €'..vehicleData.price..' cash betaald.', 'success', 3500)
        ...
        elseif ply.PlayerData.money.bank >= vehicleData.price then 
            ...
            RentedVehicles[plyCid] = plate
            TriggerClientEvent('ZyoCore:Notify', src, 'Je hebt de borg van €'..vehicleData.price..' via de bank betaald.', 'success', 3500)
            TriggerClientEvent('qb-vehiclerental:server:SpawnRentedVehicle', src, plate, vehicleData) 
        else
            TriggerClientEvent('ZyoCore:Notify', src, 'Je hebt niet genoeg geld.', 'error', 3500)
        end
        return
    end
    TriggerClientEvent('ZyoCore:Notify', src, 'Je hebt je borg van €'..vehicleData.price..' terug gekregen.', 'success', 3500)
