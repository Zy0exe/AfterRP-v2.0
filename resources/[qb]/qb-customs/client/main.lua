...
function OpenCustoms(shop, k)
    ...
    CurrentVehicleData = ZyoCore.Functions.GetVehicleProperties(veh)
    SetEntityCoords(veh, shop["coords"].x, shop["coords"].y, shop["coords"].z, 0.0, 0.0, 0.0, false)
    SetEntityHeading(veh, shop["coords"].h)
    FreezeEntityPosition(veh, true)

    if k == 7 then
        ZyoCore.Functions.TriggerCallback('qb-vehicletuning:server:IsMechanicAvailable', function(Mechanic)
            if Mechanic < 2 then
...
RegisterNUICallback('CanRepairVehicle', function(data, cb)
    ...
    ZyoCore.Functions.TriggerCallback('qb-customs:server:CanPurchase', function(CanBuy)
        if CanBuy then
	        SetVehicleFixed(veh)
            TriggerServerEvent("InteractSound_SV:PlayOnSource", "airwrench", 0.1)
            cb(true)
        else
            ZyoCore.Functions.Notify('You dont have enough money..', 'error')
            cb(false)
        end
    end, data.price)
end)
...
function GetAvailableMods()
    ...
    if ZyoCore.Shared.VehicleModels[GetEntityModel(veh)] ~= nil then
        model = ZyoCore.Shared.VehicleModels[GetEntityModel(veh)].model
        VehicleData = ZyoCore.Shared.Vehicles[model]
...
RegisterNUICallback('CloseMenu', function()
    ...
    ZyoCore.Functions.SetVehicleProperties(veh, CurrentVehicleData)
    CurrentShop = nil
end)
