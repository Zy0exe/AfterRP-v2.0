                            ...
                            if dist < 0.6 then
                                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                    ...
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        ZyoCore.Functions.TriggerCallback('qb-jewellery:server:getCops', function(cops)
                                            if cops >= Config.RequiredCops then
                                                if validWeapon() then
                                                    smashVitrine(case)
                                                else
                                                    ZyoCore.Functions.Notify('Esta arma não serve para isto..', 'error')
                                                end
                                            else
...
function smashVitrine(k)
    ...
    elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
        TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
        ZyoCore.Functions.Notify("Cortaste-te no vidro da vitrine..", "error")
    end

    smashing = true

    ZyoCore.Functions.Progressbar("smash_vitrine", "A partir a vitrine..", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
