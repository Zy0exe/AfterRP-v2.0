...
AddEventHandler('qb-jewellery:server:vitrineReward', function()
    ...
    if otherchance == odd then
        ...
        if Player.Functions.AddItem(Config.VitrineRewards[item]["item"], amount) then
            TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items[Config.VitrineRewards[item]["item"]], 'add')
        else
            TriggerClientEvent('ZyoCore:Notify', src, 'Você é ..', 'error')
        end
    else
        local amount = math.random(2, 4)
        if Player.Functions.AddItem("10kgoldchain", amount) then
            TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items["10kgoldchain"], 'add')
        else
            TriggerClientEvent('ZyoCore:Notify', src, 'Você está carregando muito..', 'error')
        end
    end
end)
...
ZyoCore.Functions.CreateCallback('qb-jewellery:server:getCops', function(source, cb)
	local amount = 0
    for k, v in pairs(ZyoCore.Functions.GetPlayers()) do
        local Player = ZyoCore.Functions.GetPlayer(v)
        if Player ~= nil then 
