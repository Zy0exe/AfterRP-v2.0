...
AddEventHandler('qb-houses:client:addGarage', function()
    if closesthouse ~= nil then 
        ...
        ZyoCore.Functions.Notify("No house around..", "error")
    end
end)
    ...
    if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                ...
                ZyoCore.Functions.Notify("House is unlocked!", "success", 2500)
            else
                TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
                ZyoCore.Functions.Notify("House is locked!", "error", 2500)
            end
        else
            ZyoCore.Functions.Notify("You dont have the keys of the house...", "error", 3500)
        end
    else
        ZyoCore.Functions.Notify("There is no door to see??", "error", 3500)
    end
end)
...
AddEventHandler('qb-houses:client:RingDoor', function(player, house)
    if closesthouse == house and inside then
        ...
        ZyoCore.Functions.Notify("Some one is ringing the door!")
    end
end)
...
AddEventHandler('qb-houses:client:giveHouseKey', function(data)
    ...
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            ...
            ZyoCore.Functions.Notify("You'r not close enough to the door..", "error")
        end
    elseif closesthouse == nil then
        ZyoCore.Functions.Notify("There is no house near you", "error")
    else
        ZyoCore.Functions.Notify("No one around!", "error")
    end
end)
...
AddEventHandler('qb-houses:client:removeHouseKey', function(data)
    if closesthouse ~= nil then 
        local housedist = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z)
        if housedist < 5 then
            ZyoCore.Functions.TriggerCallback('qb-houses:server:getHouseOwner', function(result)
                if ZyoCore.Functions.GetPlayerData().citizenid == result then
                    inHoldersMenu = true
                    HouseKeysMenu()
                    Menu.hidden = not Menu.hidden
                else
                    ZyoCore.Functions.Notify("You're not a house owner..", "error")
                end
            end, closesthouse)
        else
            ZyoCore.Functions.Notify("You'r not close enough to the door..", "error")
...
function HouseKeysMenu()
    ...
    MenuTitle = "Sleutels"
    ClearMenu()
    ZyoCore.Functions.TriggerCallback('qb-houses:server:getHouseKeyHolders', function(holders)
        ped = GetPlayerPed(-1);
        MenuTitle = "Sleutelhouders:"
        ClearMenu()
        if holders == nil or next(holders) == nil then
            ZyoCore.Functions.Notify("No key holders found..", "error", 3500)
            closeMenuFull()
        else
...
function removeOutfit(oData)
    ...
    ZyoCore.Functions.Notify(oData.outfitname.." Is removed", "success", 2500)
    closeMenuFull()
end
...
RegisterNUICallback('HasEnoughMoney', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-houses:server:HasEnoughMoney', function(hasEnough)
...
AddEventHandler('qb-houses:client:setupHouseBlips', function()
    Citizen.CreateThread(function()
        Citizen.Wait(2000)
        if isLoggedIn then
            ZyoCore.Functions.TriggerCallback('qb-houses:server:getOwnedHouses', function(ownedHouses)
...
function SetClosestHouse()
    ...
    if not inside then
        ...
        if closesthouse ~= nil then 
            ZyoCore.Functions.TriggerCallback('qb-houses:server:hasKey', function(result)
                hasKey = result
            end, closesthouse)
        
            ZyoCore.Functions.TriggerCallback('qb-houses:server:isOwned', function(result)
                isOwned = result
            end, closesthouse)
        end
    end
...
function setHouseLocations()
    if closesthouse ~= nil then
        ZyoCore.Functions.TriggerCallback('qb-houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
AddEventHandler('qb-houses:client:setLocation', function(data)
    ...
    if inside then
        if hasKey then
            ...
            ZyoCore.Functions.Notify('Je bent niet de eigenaar van het huis..', 'error')
        end
    else    
        ZyoCore.Functions.Notify('You are not in a house..', 'error')
    end
end)
...
AddEventHandler('qb-houses:client:HomeInvasion', function()
    ...
    if closesthouse ~= nil then
        ZyoCore.Functions.TriggerCallback('police:server:IsPoliceForcePresent', function(IsPresent)
            if IsPresent then
                local dist = GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true)
                        ...
                        if not Config.Houses[closesthouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('qb-houses:server:lockHouse', false, closesthouse)
                                    ZyoCore.Functions.Notify('It worked the door is now out.', 'success')
                                ...
                                ZyoCore.Functions.Notify('It faild try again.', 'error')
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('qb-houses:server:SetRamState', true, closesthouse)
                        else
                            ZyoCore.Functions.Notify('Er is al iemand bezig met de deur..', 'error')
                        end
                    else
                        ZyoCore.Functions.Notify('19/5000 This house is already open..', 'error')
                    end
                else
                    ZyoCore.Functions.Notify('You\'re not near a house..', 'error')
                end
            else
                ZyoCore.Functions.Notify('There is no police force present..', 'error')
            end
        end)
    else
        ZyoCore.Functions.Notify('You\'re not near a house..', 'error')
...
AddEventHandler('qb-houses:client:ResetHouse', function()
    ...
    if closesthouse ~= nil then
        ...
        if Config.Houses[closesthouse].IsRammed then
            ...
            TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
            RamsDone = 0
            ZyoCore.Functions.Notify('You locked the house again..', 'success')
        else
            ZyoCore.Functions.Notify('This door is not broken open  ..', 'error')
        end
    end
end)