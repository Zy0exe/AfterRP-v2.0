...
AddEventHandler('qb-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        ZyoCore.Functions.Notify(" The race endded. "..winner.. "!")
    end
end)
...
AddEventHandler('qb-streetraces:CreateRace', function(amount)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, cx, cy, groundZ, true) > 500.0 then
            ...
            ZyoCore.Functions.Notify("Race gemaakt voor €"..amount..",-!", "success")
        else
            ZyoCore.Functions.Notify("end position to close!", "error")
        end
    else
        ZyoCore.Functions.Notify("You have to place a marker!", "error")
    end
end)
...
function RaceCountDown()
    ...
    while RaceCount ~= 0 do
        ...
        ZyoCore.Functions.Notify(RaceCount, 'primary', 800)
        Citizen.Wait(1000)
    ...
    ZyoCore.Functions.Notify("GOOOOOOOOO!!!")
end
