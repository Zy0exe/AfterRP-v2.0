...
AddEventHandler('qb-streetraces:NewRace', function(RaceTable)
    ...
    local xPlayer = ZyoCore.Functions.GetPlayer(src)
    if xPlayer.Functions.RemoveMoney('cash', RaceTable.amount, "streetrace-created") then
        ...
        TriggerClientEvent('ZyoCore:Notify', src, "You joind the race for €"..Races[RaceId].amount..",-", 'success')
    end
end)
...
AddEventHandler('qb-streetraces:RaceWon', function(RaceId)
    ...
    TriggerClientEvent('ZyoCore:Notify', src, "You won the race and €"..Races[RaceId].pot..",- recieved", 'success')
    TriggerClientEvent('qb-streetraces:SetRace', -1, Races)
...
AddEventHandler('qb-streetraces:JoinRace', function(RaceId)
    local src = source
    local xPlayer = ZyoCore.Functions.GetPlayer(src)
    local zPlayer = ZyoCore.Functions.GetPlayer(Races[RaceId].creator)
    if zPlayer ~= nil then
        if xPlayer.PlayerData.money.cash >= Races[RaceId].amount then
            ...
            if xPlayer.Functions.RemoveMoney('cash', Races[RaceId].amount, "streetrace-joined") then
                TriggerClientEvent('qb-streetraces:SetRace', -1, Races)
                TriggerClientEvent('qb-streetraces:SetRaceId', src, RaceId)
                TriggerClientEvent('ZyoCore:Notify', zPlayer.PlayerData.source, GetPlayerName(src).." Joined the race", 'primary')
            end
        else
            TriggerClientEvent('ZyoCore:Notify', src, "You dont have enough cash", 'error')
        end
    else
        TriggerClientEvent('ZyoCore:Notify', src, "The person wo made the race is offline!", 'error')
        Races[RaceId] = {}
    end
end)

ZyoCore.Commands.Add("race", "A street race has started.", {{name="bedrag", help="The stake amount for the race."}}, true, function(source, args)
    local src = source
    local xPlayer = ZyoCore.Functions.GetPlayer(src)
    ...
    if GetJoinedRace(GetPlayerIdentifiers(src)[1]) == 0 then
        if xPlayer.PlayerData.money.cash >= amount then
            ...
            TriggerClientEvent('ZyoCore:Notify', src, "You don't have enough cash in your pocket", 'error')
        end
    else
        TriggerClientEvent('ZyoCore:Notify', src, "Your already in a race!", 'error')
    end
end)

ZyoCore.Commands.Add("stoprace", "Stop a race as creator.", {}, false, function(source, args)
    local src = source
    CancelRace(src)
end)

ZyoCore.Commands.Add("quitrace", "Quiting the race. (You wont get ur money back)", {}, false, function(source, args)
    local src = source
    local xPlayer = ZyoCore.Functions.GetPlayer(src)
    ...
    if RaceId ~= 0 then
        if GetCreatedRace(GetPlayerIdentifiers(src)[1]) ~= RaceId then
            ...
            TriggerClientEvent('ZyoCore:Notify', src, "You quited the race and wont get ur money back", 'error')
            TriggerClientEvent('esx:showNotification', zPlayer.PlayerData.source, GetPlayerName(src) .." has quited the race!", "red")
        else
            TriggerClientEvent('ZyoCore:Notify', src, "/stoprace to stop the race", 'error')
        end
    else
        TriggerClientEvent('ZyoCore:Notify', src, "Your not in a race.", 'error')
    end
end)

ZyoCore.Commands.Add("startrace", "Starting race", {}, false, function(source, args)
    local src = source
    ...
    if RaceId ~= 0 then
        ...
        TriggerClientEvent('ZyoCore:Notify', src, "You did not start a race", 'error')
    end
end)
            ...
            if Races[key] ~= nil and Races[key].creator == xPlayer.PlayerData.steam then
                if not Races[key].started then
                    for _, iden in pairs(Races[key].joined) do
                        local xdPlayer = ZyoCore.Functions.GetPlayer(iden)
                        xdPlayer.Functions.AddMoney('cash', Races[key].amount, "race-cancelled")
                        TriggerClientEvent('ZyoCore:Notify', xdPlayer.PlayerData.source, "The race has stopped you recieved €"..Races[key].amount..",-Back", 'error')
                        TriggerClientEvent('qb-streetraces:StopRace', xdPlayer.PlayerData.source)
                        RemoveFromRace(iden)
                    end
                else
                    TriggerClientEvent('ZyoCore:Notify', xPlayer.PlayerData.source, "The race has already started..", 'error')
                end
                TriggerClientEvent('ZyoCore:Notify', source, "Race has been stopped!", 'error')
                Races[key] = nil
            end
        end
        TriggerClientEvent('qb-streetraces:SetRace', -1, Races)
    else
        TriggerClientEvent('ZyoCore:Notify', source, "You did not start a race!", 'error')
    end
end
