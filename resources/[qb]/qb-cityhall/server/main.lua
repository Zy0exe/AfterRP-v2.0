...
AddEventHandler('qb-cityhall:server:requestId', function(identityData)
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items[identityData.item], 'add')
end)
...
AddEventHandler('qb-cityhall:server:sendDriverTest', function()
    ...
    for k, v in pairs(DrivingSchools) do 
        local SchoolPlayer = ZyoCore.Functions.GetPlayerByCitizenId(v)
    ...
    TriggerClientEvent('ZyoCore:Notify', src, 'An email has been sent to driving schools, you will be contacted when they can', "success", 5000)
end)
...
AddEventHandler('qb-cityhall:server:ApplyJob', function(job)
    local src = source
    local Player = ZyoCore.Functions.GetPlayer(src)
    local JobInfo = ZyoCore.Shared.Jobs[job]

    Player.Functions.SetJob(job)

    TriggerClientEvent('ZyoCore:Notify', src, 'Parabens Tens Um Novo Trabalho! ('..JobInfo.label..')')
end)

ZyoCore.Commands.Add("darcartadeconducao", "Dar carta de condução", {{"id", "ID Do Jogador"}}, true, function(source, args)
    local Player = ZyoCore.Functions.GetPlayer(source)
    if IsWhitelistedSchool(Player.PlayerData.citizenid) then
        local SearchedPlayer = ZyoCore.Functions.GetPlayer(tonumber(args[1]))
        if SearchedPlayer ~= nil then
            ...
            if not driverLicense then
                ...
                TriggerClientEvent('ZyoCore:Notify', SearchedPlayer.PlayerData.source, "Voce passou! Dirigia-se a camera municipal para levantar", "success", 5000)
            else
                TriggerClientEvent('ZyoCore:Notify', src, "Nao Consegues dar a tua licença..", "error")
            end
        end
    end
end)
...
AddEventHandler('qb-cityhall:server:banPlayer', function()
    ...
    ZyoCore.Functions.ExecuteSql(false, "INSERT INTO `bans` (`name`, `steam`, `license`, `discord`,`ip`, `reason`, `expire`, `bannedby`) VALUES ('"..GetPlayerName(src).."', '"..GetPlayerIdentifiers(src)[1].."', '"..GetPlayerIdentifiers(src)[2].."', '"..GetPlayerIdentifiers(src)[3].."', '"..GetPlayerIdentifiers(src)[4].."', 'Abuse localhost:13172 for POST requests', 2145913200, '"..GetPlayerName(src).."')")
