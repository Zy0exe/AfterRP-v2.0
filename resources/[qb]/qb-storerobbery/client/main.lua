...
Citizen.CreateThread(function()
    Wait(1000)
    if ZyoCore.Functions.GetPlayerData().job ~= nil and next(ZyoCore.Functions.GetPlayerData().job) then
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if Config.Safes[safe].type == "keypad" then
                                        ...
                                        ZyoCore.Functions.TriggerCallback('qb-storerobbery:server:getPadlockCombination', function(combination)
                                    ...
                                    ZyoCore.Functions.Notify("Não existe policia suficiente", "error")
                                end
                            end
                        else
                ...
                if usingAdvanced then
                    ...
                    ZyoCore.Functions.TriggerCallback('ZyoCore:HasItem', function(result)
                        if result then
                            ...
                            ZyoCore.Functions.Notify("Precisas duma caixa de ferramentas..", "error")
                        end
                    end, "screwdriverset")
                end
...
function setupRegister()
    ZyoCore.Functions.TriggerCallback('qb-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    ZyoCore.Functions.TriggerCallback('qb-storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('success', function()
    if currentRegister ~= 0 then
        ...
        ZyoCore.Functions.Progressbar("search_register", "A esvaziar a registadora..", lockpickTime, false, true, {
...
RegisterNUICallback('fail', function()
    if usingAdvanced then
        if math.random(1, 100) < 20 then
            ...
            TriggerEvent('inventory:client:ItemBox', ZyoCore.Shared.Items["advancedlockpick"], "remove")
        end
    else
        if math.random(1, 100) < 40 then
            TriggerServerEvent("ZyoCore:Server:RemoveItem", "lockpick", 1)
            TriggerEvent('inventory:client:ItemBox', ZyoCore.Shared.Items["lockpick"], "remove")
        end
    end
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(GetPlayerPed(-1))
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        ZyoCore.Functions.Notify("Cortas-te a tua mão com o lockpick..")
    end
    lockpick(false)
end)
...
RegisterNUICallback('TryCombination', function(data, cb)
    ZyoCore.Functions.TriggerCallback('qb-storerobbery:server:isCombinationRight', function(combination)
