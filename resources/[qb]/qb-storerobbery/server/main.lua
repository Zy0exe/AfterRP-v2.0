...
AddEventHandler('qb-storerobbery:server:SafeReward', function(safe)
    ...
    if luck <= 10 then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items["rolex"], "add")
        if luck == odd then
            Citizen.Wait(500)
...
ZyoCore.Functions.CreateCallback('qb-storerobbery:server:isCombinationRight', function(source, cb, safe)
    cb(SafeCodes[safe])
end)

ZyoCore.Functions.CreateCallback('qb-storerobbery:server:getPadlockCombination', function(source, cb, safe)
    cb(SafeCodes[safe])
end)

ZyoCore.Functions.CreateCallback('qb-storerobbery:server:getRegisterStatus', function(source, cb)
    cb(Config.Registers)
end)

ZyoCore.Functions.CreateCallback('qb-storerobbery:server:getSafeStatus', function(source, cb)
    cb(Config.Safes)
end)