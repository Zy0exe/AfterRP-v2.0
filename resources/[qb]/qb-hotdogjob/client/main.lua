...
function StartWorking()
    ZyoCore.Functions.TriggerCallback('qb-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            local PlayerPed = GetPlayerPed(-1)
            ...
            UpdateUI()
            CheckLoop()
            ZyoCore.Functions.Notify('Você pagou um depósito de 1000, -!..', 'success')
        else
            ZyoCore.Functions.Notify('Você não tem dinheiro para o depósito..', 'error')
        end
    end)
end
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
                ...
                while true do
                    if SellingData.Enabled then
                        ...
                        if not SellingData.HasTarget then
                            ...
                            local closestPed, closestDistance = ZyoCore.Functions.GetClosestPed(coords, PlayerPeds)
            ...
            ZyoCore.Functions.Notify('Você está muito longe da sua barraca de cachorro-quente..', 'error')
        end
    else
        ZyoCore.Functions.Notify('Você não tem barraca de cachorro-quente', 'error')
    end
end
...
function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
        ...
        if PlayerDist > 15.0 then
            ...
            ZyoCore.Functions.Notify('Você está muito longe da baia..', 'error')
            break
        end
        Citizen.Wait(100)
    end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 7.5 then
            if SellingData.Hotdog ~= nil then
                ...
                ZyoCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Venda '..HotdogsForSale..'X para €'..(HotdogsForSale * SellingPrice)..',- / [8] Rejeitar')
                if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                    ZyoCore.Functions.Notify(HotdogsForSale..'x Cachorro-quente vendido por €'..(HotdogsForSale * SellingPrice)..',-', 'success')
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    ZyoCore.Functions.Notify('Cliente recusou!', 'error')
                    SellingData.HasTarget = false
                ...
                SellingData.Hotdog = GetAvailableHotdog()
                ZyoCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, 'Você não tem cachorro-quente.. / [8] Recusar cliente')
            ...
            FreezeEntityPosition(ped, false)
            SetPedKeepTask(ped, false)
            SetEntityAsNoLongerNeedeZyoCore = nil
local isLoggedIn = false
PlayerData = {}
PlayerJob = {}
...
function StartWorking()
    ZyoCore.Functions.TriggerCallback('qb-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            ...
            UpdateUI()
            CheckLoop()
            ZyoCore.Functions.Notify('You have paid a € 1000 deposit!..', 'success')
        else
            ZyoCore.Functions.Notify('You have no money for the deposit..', 'error')
        end
    end)
end
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            ZyoCore.Functions.Notify('You are too far from your Hot Dog Stand..', 'error')
        end
    else
        ZyoCore.Functions.Notify('You don\'t have a Hotdog stand', 'error')
    end
end
...
function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
        ...
        if PlayerDist > 15.0 then
            ...
            ZyoCore.Functions.Notify('You are too far from your stall..', 'error')
            break
        end
        Citizen.Wait(100)
    end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 7.5 then
            if SellingData.Hotdog ~= nil then
                ...
                ZyoCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Verkoop '..HotdogsForSale..'x voor €'..(HotdogsForSale * SellingPrice)..',- / [8] Afwijzen')
                if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                    ZyoCore.Functions.Notify(HotdogsForSale..'x Hotdog(\'s) sold for €'..(HotdogsForSale * SellingPrice)..',-', 'success')
                    TriggerServerEvent('qb-hotdogjob:server:Sell', HotdogsForSale, SellingPrice)
                    SellingData.HasTarget = false
                    local Myped = GetPlayerPed(-1)
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    ZyoCore.Functions.Notify('Customer refused!', 'error')
                    SellingData.HasTarget = false
                ...
                SellingData.Hotdog = GetAvailableHotdog()
                ZyoCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, 'You have no stock .. / [8] Refuse customer')
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        ...
        if Config.MyLevel == 1 then
            ZyoCore.Functions.Notify('You have made a '..Config.Stock[Quality].Label..' Hotdog!')
            ...
            if Luck == LuckyNumber then
                ZyoCore.Functions.Notify('you have '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hotdog\'s made!')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + LuckyAmount
            else
                ZyoCore.Functions.Notify('You have made '..Config.Stock[Quality].Label..' a Hotdog !')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + 1
            end
        end
    else
        ZyoCore.Functions.Notify('You do not have ('..Config.Stock[Quality].Label..') stock more about it..')
    end
    PreparingFood = false
end
...
function StopWorking()
    if DoesEntityExist(StandObject) then
        ZyoCore.Functions.TriggerCallback('qb-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                ZyoCore.Functions.Notify('You have received your € 1000 deposit back!', 'success')
            else
                ZyoCore.Functions.Notify('Something is wrong..', 'error')
            end
        end)
    else
        ZyoCore.Functions.Notify('Your hot dog stall was nowhere to be seen, you did NOT receive your deposit back!', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
...
AddEventHandler('qb-hotdogjob:staff:DeletStand', function()
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Citizen.Wait(100)            
            DeleteEntity(Object)
            ZyoCore.Functions.Notify('Hot dog mode removed!')
        end
    end
end)
...
endd(ped)
            ...
            ZyoCore.Functions.Notify('Você está muito longe do local..', 'error')
            break
        end
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        TriggerServerEvent('qb-hotdogjob:server:UpdateReputation', Quality)
        if Config.MyLevel == 1 then
            ZyoCore.Functions.Notify('Você fez um  '..Config.Stock[Quality].Label..' Hotdog !')
            Config.Stock[Quality].Current = Config.Stock[Quality].Current + 1
        else
            local Luck = math.random(1, 2)
            local LuckyNumber = math.random(1, 2)
            local LuckyAmount = math.random(1, Config.MyLevel)
            if Luck == LuckyNumber then
                ZyoCore.Functions.Notify('Você tem '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hotdog\'s !')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + LuckyAmount
            else
                ZyoCore.Functions.Notify('Você fez um  '..Config.Stock[Quality].Label..' Hotdog !')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + 1
        ...
        ZyoCore.Functions.Notify('Você tem  ('..Config.Stock[Quality].Label..') nenhum cachorro-quente..')
    end
    PreparingFood = false
end
    ...
    if DoesEntityExist(StandObject) then
        ZyoCore.Functions.TriggerCallback('qb-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                ZyoCore.Functions.Notify('Você recebe seus 1000€, - do deposito!', 'success')
            else
                ZyoCore.Functions.Notify('Algo está errado meus amigos..', 'error')
            end
        end)
    else
        ZyoCore.Functions.Notify('Sua barraca não é onde ver você não receberá seu depósito', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
...
AddEventHandler('qb-hotdogjob:staff:DeletStand', function()
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Citizen.Wait(100)            
            DeleteEntity(Object)
            ZyoCore.Functions.Notify('Barraca de cachorro-quente removida!')
        end
    end
end)
