                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    gender = "male"
                                    if ZyoCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
                                    ZyoCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
...
AddEventHandler("qb-clothing:client:adjustfacewear",function(type)
    if ZyoCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
