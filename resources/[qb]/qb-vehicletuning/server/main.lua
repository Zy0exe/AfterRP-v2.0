...
ZyoCore.Functions.CreateCallback('qb-vehicletuning:server:GetDrivingDistances', function(source, cb)
...
AddEventHandler('qb-vehicletuning:server:UpdateDrivingDistance', function(amount, plate)
    ...
    ZyoCore.Functions.ExecuteSql(false, "SELECT * FROM `player_vehicles` WHERE `plate` = '"..plate.."'", function(result)
        if result[1] ~= nil then
            ZyoCore.Functions.ExecuteSql(false, "UPDATE `player_vehicles` SET `drivingdistance` = '"..amount.."' WHERE `plate` = '"..plate.."'")
        end
    end)
end)

ZyoCore.Functions.CreateCallback('qb-vehicletuning:server:IsVehicleOwned', function(source, cb, plate)
    local retval = false
    ZyoCore.Functions.ExecuteSql(false, "SELECT * FROM `player_vehicles` WHERE `plate` = '"..plate.."'", function(result)
        if result[1] ~= nil then
            retval = true
...
function GetVehicleStatus(plate)
    local retval = nil
    ZyoCore.Functions.ExecuteSql(true, "SELECT `status` FROM `player_vehicles` WHERE `plate` = '"..plate.."'", function(result)
        if result[1] ~= nil then
...
ZyoCore.Commands.Add("setvehiclestatus", "Zet vehicle status", {{name="part", help="Type status dat je wilt bewerken"}, {name="amount", help="Level van de status"}}, true, function(source, args)
    local part = args[1]:lower()
    local level = tonumber(args[2])
    TriggerClientEvent("vehiclemod:client:setPartLevel", source, part, level)
end, "god")

ZyoCore.Functions.CreateCallback('qb-vehicletuning:server:GetAttachedVehicle', function(source, cb)
    cb(Config.Plates)
end)

ZyoCore.Functions.CreateCallback('qb-vehicletuning:server:IsMechanicAvailable', function(source, cb)
	local amount = 0
    ...
    if RepairPart ~= nil then
        if RepairPart.amount >= Config.RepairCostAmount[part].costs then
            ...
            for i = 1, Config.RepairCostAmount[part].costs, 1 do
                TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items[Config.RepairCostAmount[part].item], "remove")
                Citizen.Wait(500)
            end
        else
            TriggerClientEvent('ZyoCore:Notify', src, "Você não tem o suficiente "..ZyoCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." (min. "..Config.RepairCostAmount[part].costs.."x)", "error")
        end
    else
        TriggerClientEvent('ZyoCore:Notify', src, "Você não tem "..ZyoCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." contigo!", "error")
    end
end)
...
ZyoCore.Commands.Add("contratarmecanico", "Dê a alguém um emprego de mecânico", {{name="id", help="ID Do Jogador"}}, false, function(source, args)
    local Player = ZyoCore.Functions.GetPlayer(source)

    if IsAuthorized(Player.PlayerData.citizenid) then
        local TargetId = tonumber(args[1])
        if TargetId ~= nil then
            local TargetData = ZyoCore.Functions.GetPlayer(TargetId)
            if TargetData ~= nil then
                TargetData.Functions.SetJob("mechanic")
                TriggerClientEvent('ZyoCore:Notify', TargetData.PlayerData.source, "Você contratou como funcionário da Auto Care!")
                TriggerClientEvent('ZyoCore:Notify', source, "Você tem ("..TargetData.PlayerData.charinfo.firstname..") contratado como funcionário da Auto Care!")
            end
        else
            TriggerClientEvent('ZyoCore:Notify', source, "Você deve fornecer um ID de jogador!")
        end
    else
        TriggerClientEvent('ZyoCore:Notify', source, "Você não pode fazer isso!", "error") 
    end
end)

ZyoCore.Commands.Add("despedirmecanico", "Tirar o emprego de mecânico de alguém", {{name="id", help="ID Do Jogador"}}, false, function(source, args)
    local Player = ZyoCore.Functions.GetPlayer(source)
        ...
        if TargetId ~= nil then
            ...
            if TargetData ~= nil then
                if TargetData.PlayerData.job.name == "mechanic" then
                    ...
                    TriggerClientEvent('ZyoCore:Notify', TargetData.PlayerData.source, "Você foi demitido como funcionário da Auto Care!")
                    TriggerClientEvent('ZyoCore:Notify', source, "Você foi ("..TargetData.PlayerData.charinfo.firstname..") despedido como funcionário da Auto Care!")
                else
                    TriggerClientEvent('ZyoCore:Notify', source, "Este não é um funcionário da Autocare!", "error")
                end
            end
        else
            TriggerClientEvent('ZyoCore:Notify', source, "Você deve fornecer um ID de jogador!", "error")
        end
    else
        TriggerClientEvent('ZyoCore:Notify', source, "Você não pode fazer isso!", "error")
    end
...
ZyoCore.Functions.CreateCallback('qb-vehicletuning:server:GetStatus', function(source, cb, plate)
