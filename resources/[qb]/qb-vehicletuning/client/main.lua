...
AddEventHandler('ZyoCore:Client:OnPlayerLoaded', function()
    ZyoCore.Functions.GetPlayerData(function(PlayerData)
        PlayerJob = PlayerData.job
        if PlayerData.job.onduty then
            if PlayerData.job.name == "mechanic" then
                TriggerServerEvent("ZyoCore:ToggleDuty")
            end
        end
    end)
    isLoggedIn = true
    ZyoCore.Functions.TriggerCallback('qb-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    ZyoCore.Functions.TriggerCallback('qb-vehicletuning:server:GetDrivingDistances', function(retval)
...
RegisterNetEvent('ZyoCore:Client:SetDuty')
AddEventHandler('ZyoCore:Client:SetDuty', function(duty)
    onDuty = duty
end)
                        ...
                        if VehicleDistance < 1 then
                            ...
                            if InVehicle then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ZyoCore.Functions.DeleteVehicle(GetVehiclePedIsIn(GetPlayerPed(-1)))
                                end
                            else
                                ...
                                if PlateDistance < 2 then
                                    ...
                                    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            ...
                                            ZyoCore.Functions.Notify("Não podes colocar uma bicicleta na plataforma!", "error")
                                        end
                                    end
                                end
                            end
...
function SpawnListVehicle(model)
    ...
    ZyoCore.Functions.SpawnVehicle(model, function(veh)
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Reparar ("..ZyoCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Voltar", "VehicleOptions", nil)
    Menu.addButton("Fechar Menu", "CloseMenu", nil) 
end
...
function RepairPart(part)
    ...
    ZyoCore.Functions.TriggerCallback('qb-inventory:server:GetStashItems', function(StashItems)
        for k, v in pairs(StashItems) do
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    ZyoCore.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." A ser reparado", math.random(5000, 10000), false, true, {
                    ...
                    ZyoCore.Functions.Notify('Não tens materiais suficientes no armario..', 'error')
                end
                break
            else
                ZyoCore.Functions.Notify('Não tens materiais suficientes no armario..', 'error')
...
AddEventHandler('qb-vehicletuning:client:RepaireeePart', function(part)
    ...
    ZyoCore.Functions.Notify("De "..Config.ValuesLabels[part].." foi reparado!")
end)
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        ZyoCore.Functions.Notify("Estado desconhecido..", "error")
                    end
                else
                    ZyoCore.Functions.Notify("Veiculo invalido..", "error")
                end
            else
                ZyoCore.Functions.Notify("Não estas perto suficiente do veiculo..", "error")
            end
        else
            ZyoCore.Functions.Notify("Tens que entrar no veiculo primeiro..", "error")
        end
    else
        ZyoCore.Functions.Notify("Tens que estar fora do veiculo..", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(GetPlayerPed(-1), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) then
            ...
            ZyoCore.Functions.Notify("Não estas no lugar de condutor do veiculo..", "error")
            end
        else
            ZyoCore.Functions.Notify("Não estas num veiculo..", "error")
    end
end)
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            ZyoCore.Functions.Progressbar("repair_advanced", "A reparar o veiculo..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                if part == "body" then
                                    ...
                                    TriggerServerEvent("ZyoCore:Server:RemoveItem", Config.RepairCost[part], needAmount)
                                    TriggerEvent("inventory:client:ItemBox", ZyoCore.Shared.Items[Config.RepairCost[part]], "remove")
                                elseif part ~= "engine" then
                                    TriggerServerEvent("vehiclemod:server:updatePart", plate, part, GetVehicleStatus(plate, part) + level)
                                    TriggerServerEvent("ZyoCore:Server:RemoveItem", Config.RepairCost[part], level)
                                    TriggerEvent("inventory:client:ItemBox", ZyoCore.Shared.Items[Config.RepairCost[part]], "remove")
                                end
                            end, function() -- Cancel
                                openingDoor = false
                                ClearPedTasks(GetPlayerPed(-1))
                                ZyoCore.Functions.Notify("Processo cancelado..", "error")
                            end)
                        else
                            ZyoCore.Functions.Notify("Componente invalido.", "error")
                        end
                    else
                        ZyoCore.Functions.Notify("Veiculo invalido..", "error")
                    end
                else
                    ZyoCore.Functions.Notify("Não estas perto do veiculo..", "error")
                end
            else
                ZyoCore.Functions.Notify("Tens que entrar no veiculo primeiro..", "error")
            end
...
function SendStatusMessage(statusList)
    if statusList ~= nil then 
        TriggerEvent('chat:addMessage', {
            ...
            args = {'Voertuig Status', round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..ZyoCore.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..ZyoCore.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..ZyoCore.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..ZyoCore.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..ZyoCore.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..ZyoCore.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..ZyoCore.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end
