-- Load ZyoCore and get permissions --
ZyoCore = nil
...
AddEventHandler('ZyoCore:Client:OnPlayerLoaded', function()
    ZyoCore.Functions.TriggerCallback('qb-anticheat:server:GetPermissions', function(UserGroup)
        group = UserGroup
    end)
    isLoggedIn = true
end)
        ...
        if group == Config.Group and isLoggedIn then 
            if not IsDecorating then 
                if not IsEntityVisible(ped) then
                    ...
                    TriggerEvent('ZyoCore:Notify', "ANTICHEAT: Estavas invisivel e foste tornado visivel outra vez!")
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            ...
            local WeaponInformation = ZyoCore.Shared.Weapons[CurrentWeapon]

            if WeaponInformation["name"] ~= "weapon_unarmed" then
                ZyoCore.Functions.TriggerCallback('qb-anticheat:server:HasWeaponInInventory', function(HasWeapon)
