...
AddEventHandler('qb-log:server:CreateLog', function(name, title, color, message, tagEveryone)
    if source ~= 0 then  -- Prevents everyone abusing logs.
        ...
        PerformHttpRequest(webHook, function(err, text, headers) end, 'POST', json.encode({ username = "Zyo AFTER V2.0",embeds = embedData}), { ['Content-Type'] = 'application/json' })
        Citizen.Wait(100)
        if tag then
            PerformHttpRequest(webHook, function(err, text, headers) end, 'POST', json.encode({ username = "Zyo AFTER V2.0", content = "@everyone"}), { ['Content-Type'] = 'application/json' })
        end
    end
end)
...
ZyoCore.Commands.Add("testwebhook", ":)", {}, false, function(source, args)
