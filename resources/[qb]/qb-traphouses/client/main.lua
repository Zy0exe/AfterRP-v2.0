...
Citizen.CreateThread(function()
    ...
    if ZyoCore.Functions.GetPlayerData() ~= nil then
        ...
        ZyoCore.Functions.TriggerCallback('qb-traphouses:server:GetTraphousesData', function(trappies)
...
RegisterNUICallback('EnterPincode', function(d)
    ...
    if tonumber(d.pin) == data.pincode then
        ...
        ZyoCore.Functions.Notify('This code is not valide..', 'error')
    end
end)
                        ...
                        if not IsKeyHolder then
                            ...
                            if IsHouseOwner then
                                ...
                                if IsControlJustPressed(0, Keys["G"]) then
                                    ZyoCore.Functions.Notify('Pincode: '..data.pincode)
                                end
                            end
                            if IsControlJustPressed(0, Keys["H"]) then
                                local TraphouseInventory = {}
...
AddEventHandler('qb-traphouses:client:TakeoverHouse', function(TraphouseId)
    ...
    ZyoCore.Functions.Progressbar("takeover_traphouse", "Overtaking traphouse", math.random(1000, 3000), false, true, {
        ...
        ZyoCore.Functions.Notify("Overtaking canceled..", "error")
    end)
end)
...
function AddKeyHolder(CitizenId, Traphouse)
    if #Config.TrapHouses[Traphouse].keyholders <= 6 then
        if not HasCitizenIdHasKey(CitizenId, Traphouse) then
            ...
            ZyoCore.Functions.Notify(CitizenId..' is added to the traphouse!')
        else
            ZyoCore.Functions.Notify(CitizenId..' This person does already have the keys!')
        end
    else
        ZyoCore.Functions.Notify('You can max give 6 other people acces to the trap house!')
    end
