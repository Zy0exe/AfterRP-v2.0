    ...
    if Config.TrapHouses[TraphouseId] ~= nil then
        ...
        if Config.TrapHouses[TraphouseId].keyholders == nil then
            ...
            if #Config.TrapHouses[TraphouseId].keyholders + 1 <= 6 then
                ...
                TriggerClientEvent('ZyoCore:Notify', src, 'There arent any slots over..')
            end
        end
    else
        TriggerClientEvent('ZyoCore:Notify', src, 'Error has occurred..')
    end
end)
...
ZyoCore.Commands.Add("entertraphouse", "Betreed traphouse", {}, false, function(source, args)
    local src = source
    local Player = ZyoCore.Functions.GetPlayer(src)
...
ZyoCore.Commands.Add("givetrapkeys", "Give's the keys of the trap house", {{name = "id", help = "Speler id"}}, true, function(source, args)
    local src = source
    local Player = ZyoCore.Functions.GetPlayer(src)
    ...
    if TargetData ~= nil then
        if Traphouse ~= nil then
            if not HasCitizenIdHasKey(TargetData.PlayerData.citizenid, Traphouse) then
                if Config.TrapHouses[Traphouse] ~= nil then
                    ...
                    if Config.TrapHouses[Traphouse].keyholders == nil then
                        ...
                        if #Config.TrapHouses[Traphouse].keyholders + 1 <= 6 then
                            ...
                            TriggerClientEvent('ZyoCore:Notify', src, 'there arent any slots left over..')
                        end
                    end
                else
                    TriggerClientEvent('ZyoCore:Notify', src, 'Error has occurred..')
                ...
                TriggerClientEvent('ZyoCore:Notify', src, 'This person already owns a key..', 'error')
            end
        else
            TriggerClientEvent('ZyoCore:Notify', src, 'You dont own the trap house or you dont own one at all..', 'error')
        end
    else
...
AddEventHandler('qb-traphouses:server:TakeMoney', function(TraphouseId)
    ...
    if Config.TrapHouses[TraphouseId].money ~= 0 then
        ...
        TriggerClientEvent('ZyoCore:Notify', src, 'There issent any money in the cupboard', 'error')
    end
end)
...
function AddHouseItem(traphouseId, slot, itemName, amount, info, source)
    ...
    if Config.TrapHouses[traphouseId].inventory[slot] ~= nil and Config.TrapHouses[traphouseId].inventory[slot].name == itemName then
        ...
        local itemInfo = ZyoCore.Shared.Items[itemName:lower()]
...
AddEventHandler('qb-traphouses:server:RobNpc', function(Traphouse)
    ...
    if Chance == odd then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items["stickynote"], "add")
    else
...
ZyoCore.Functions.CreateCallback('qb-traphouses:server:GetTraphousesData', function(source, cb)
