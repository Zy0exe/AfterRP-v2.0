...
Citizen.CreateThread(function()
    while true do
        ...
        if ZyoCore ~= nil then
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), true), -1) == GetPlayerPed(-1) then
                ...
                if LastVehicle ~= GetVehiclePedIsIn(GetPlayerPed(-1), false) then
                    ZyoCore.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
                        if result then
                            HasKey = true
        ...
        if not HasKey and IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) and ZyoCore ~= nil and not IsHotwiring then
            local veh = GetVehiclePedIsIn(GetPlayerPed(-1), false)
            SetVehicleEngineOn(veh, false, false, true)
            --[[local veh = GetVehiclePedIsIn(GetPlayerPed(-1), false)
            local vehpos = GetOffsetFromEntityInWorldCoords(veh, 0, 1.5, 0.5)
            ZyoCore.Functions.DrawText3D(vehpos.x, vehpos.y, vehpos.z, "~g~H~w~ - Hotwire")
            SetVehicleEngineOn(veh, false, false, true)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(7)
        if not IsRobbing and isLoggedIn and ZyoCore ~= nil then
            if GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= nil and GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= 0 then
                ...
                if driver ~= 0 and not IsPedAPlayer(driver) then
                    if IsEntityDead(driver) then
                        IsRobbing = true
                        ZyoCore.Functions.Progressbar("rob_keys", "A pegar nas chaves do veiculo..", 3000, false, true, {}, {}, {}, {}, function() -- Done
            ...
            --[[if ZyoCore.Functions.GetPlayerData().job.name ~= "police" then
...
AddEventHandler('vehiclekeys:client:SetOwner', function(plate)
    ...
    HasKey = true
    --ZyoCore.Functions.Notify('You picked the keys of the vehicle', 'success', 3500)
end)
...
function LockVehicle()
    local veh = ZyoCore.Functions.GetClosestVehicle()
    ...
    if veh ~= nil and GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 7.5 then
        ZyoCore.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
            if result then
                if HasKey then
                    ...
                    if vehLockStatus == 1 then
                        ...
                        if(GetVehicleDoorLockStatus(veh) == 2)then
                            ZyoCore.Functions.Notify("Trancaste o veiculo!")
                        else
                            ZyoCore.Functions.Notify("Ocorreu um erro com o sistema de tranque!")
                        end
                    else
                        Citizen.Wait(750)
                        ClearPedTasks(GetPlayerPed(-1))
                        ...
                        if(GetVehicleDoorLockStatus(veh) == 1)then
                            ZyoCore.Functions.Notify("Destrancaste o veiculo!")
                        else
                            ZyoCore.Functions.Notify("Ocorreu um erro com o sistema de tranque!")
                        end
                    end
                ...
                ZyoCore.Functions.Notify('Não tens as chaves deste veiculo..', 'error')
            end
        end, plate)
    else
    	print("TRANCAR: NENHUM VEICULO À TUA FRENTE")
    end
end
    ...
    if vehicle ~= nil and vehicle ~= 0 then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 1.5 then
            ...
            if (vehLockStatus > 1) then
                ...
                ZyoCore.Functions.Progressbar("lockpick_vehicledoor", "A arrombar a porta..", lockpickTime, false, true, {
                    ...
                    if math.random(1, 100) <= 90 then
                        ZyoCore.Functions.Notify("Conseguiste abrir a porta!")
                        TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5, "unlock", 0.3)
                        SetVehicleDoorsLocked(vehicle, 0)
                        SetVehicleDoorsLockedForAllPlayers(vehicle, false)
                    else
                        ZyoCore.Functions.Notify("Falhou!", "error")
                    end
                end, function() -- Cancel
                    openingDoor = false
                    StopAnimTask(GetPlayerPed(-1), "veh@break_in@0h@p_m_one@", "low_force_entry_ds", 1.0)
                ...
                Skillbar.Start({
                    ...
                    if IsHotwiring then
                        if SucceededAttempts + 1 >= NeededAttempts then
                            ...
                            ZyoCore.Functions.Notify("Fizeste o lockpick com sucesso!")
                            HasKey = true
                            TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle))
                            IsHotwiring = false
                            FailedAttemps = 0
                            ...
                            if vehicle ~= nil and vehicle ~= 0 then
                                ...
                                ZyoCore.Functions.Notify("Tens que estar dentro de um veiculo", "error")
                                IsHotwiring = false
                    ...
                    if IsHotwiring then
                        ...
                        HasKey = false
                        SetVehicleEngineOn(vehicle, false, false, true)
                        ZyoCore.Functions.Notify("Não conseguiste fazer o lockpick!", "error")
                        IsHotwiring = false
                        FailedAttemps = FailedAttemps + 1
                        local c = math.random(2)
                        local o = math.random(2)
...
function Hotwire()
    if not HasKey then 
        ...
        PoliceCall()
        ZyoCore.Functions.Progressbar("hotwire_vehicle", "A fazer ligação direta", hotwireTime, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            if (math.random(0, 100) < 10) then
                HasKey = true
                ZyoCore.Functions.Notify("Conseguiste fazer ligação direta!")
            else
                HasKey = false
                SetVehicleEngineOn(veh, false, false, true)
                ZyoCore.Functions.Notify("Não conseguiste fazer ligação direta!", "error")
            end
            IsHotwiring = false
        end, function() -- Cancel
            StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
            HasKey = false
            SetVehicleEngineOn(veh, false, false, true)
            ZyoCore.Functions.Notify("Falhaste a ligação direta!", "error")
            IsHotwiring = false
        end)
    end
end
        ...
        if math.random(1, 100) <= chance then
            ...
            if closestPed ~= nil then
                ...
                if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                    ...
                    if ZyoCore.Shared.VehicleModels[modelName] ~= nil then
                        Name = ZyoCore.Shared.Vehicles[ZyoCore.Shared.VehicleModels[modelName]["model"]]["brand"] .. ' ' .. ZyoCore.Shared.Vehicles[ZyoCore.Shared.VehicleModels[modelName]["model"]]["name"]
                    else
                        Name = "Desconhecido"
                    end
