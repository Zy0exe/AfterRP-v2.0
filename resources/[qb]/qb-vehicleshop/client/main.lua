        ...
        if v["shop"] == "pdm" then
            for cat,_ in pairs(vehicleCategorys) do
                if ZyoCore.Shared.Vehicles[k]["category"] == cat then
                    table.insert(vehicleCategorys[cat].vehicles, ZyoCore.Shared.Vehicles[k])
                end
            end
        end
    end
...
--                     if dist <= 1.5 then
--                         ZyoCore.Functions.DrawText3D(QB.VehicleShops[k].x, QB.VehicleShops[k].y, QB.VehicleShops[k].z + 1.3, '~g~E~w~ - Premium Deluxe Motorsports')
