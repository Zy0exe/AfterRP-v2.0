    ...
    while true do
        ...
        if SellDistance < 20 then
            if IsPedInAnyVehicle(ped) then
                local VehicleData = ZyoCore.Shared.VehicleModels[GetEntityModel(GetVehiclePedIsIn(ped))]
                    ...
                    if SellDistance < 1 then
                        if not SellStarted then
                            ...
                            if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                                SellStarted = false
                                ZyoCore.Functions.TriggerCallback('qb-vehicleshop:server:SellVehicle', function(SoldVehicle)
                                    if SoldVehicle then
                                        ...
                                        ZyoCore.Functions.Notify('Este veiculo não te pertence..', 'error')
                                    end
        ...
        if ClosestVehicle ~= nil then
            if dist < 1.5 then
                if not QB.ShowroomVehicles[ClosestVehicle].inUse then
                    ...
                    local displayName = ZyoCore.Shared.Vehicles[QB.ShowroomVehicles[ClosestVehicle].chosenVehicle]["name"]
                    local vehPrice = ZyoCore.Shared.Vehicles[QB.ShowroomVehicles[ClosestVehicle].chosenVehicle]["price"]
                    ...
                    if IsDisabledControlJustPressed(0, Keys["7"]) then
                        if buySure then
                            local class = ZyoCore.Shared.Vehicles[QB.ShowroomVehicles[ClosestVehicle].chosenVehicle]["category"]
                    ...
                    if IsDisabledControlJustPressed(0, Keys["8"]) then
                        ZyoCore.Functions.Notify('Cancelaste a compra do veiculo', 'error', 3500)
                        buySure = false
                    end
...
AddEventHandler('qb-vehicleshop:client:setShowroomVehicle', function(showroomVehicle, k)
    if QB.ShowroomVehicles[k].chosenVehicle ~= showroomVehicle then
        ZyoCore.Functions.DeleteVehicle(GetClosestVehicle(QB.ShowroomVehicles[k].coords.x, QB.ShowroomVehicles[k].coords.y, QB.ShowroomVehicles[k].coords.z, 3.0, 0, 70))
        modelLoaded = false
        Wait(250)
...
AddEventHandler('qb-vehicleshop:client:buyShowroomVehicle', function(vehicle, plate)
    ZyoCore.Functions.SpawnVehicle(vehicle, function(veh)
