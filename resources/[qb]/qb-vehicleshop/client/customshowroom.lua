                    ...
                    if dist < 15 then
                        ...
                        if dist < 2 then
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                ZyoCore.Functions.DeleteVehicle(veh)
                            end
                        end
                    end
                end
...
Citizen.CreateThread(function()
    Citizen.Wait(1000)
    for k, v in pairs(ZyoCore.Shared.Vehicles) do
        if v["shop"] == "custom" then
            for cat,_ in pairs(CustomVehicleCats) do
                if ZyoCore.Shared.Vehicles[k]["category"] == cat then
                    table.insert(CustomVehicleCats[cat].vehicles, ZyoCore.Shared.Vehicles[k])
                end
            end
        end
    end
                                ...
                                if QBCustom.ShowroomPositions[ClosestCustomVehicle].buying then
                                    if ZyoCore.Shared.Vehicles[QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle] ~= nil then
                                        ...
                                        if IsDisabledControlJustPressed(0, Keys["8"]) then
                                            ZyoCore.Functions.Notify('You diddent buy the vehicle!')
                                            QBCustom.ShowroomPositions[ClosestCustomVehicle].buying = false
                                        end
                                    end
                                end
...
AddEventHandler('qb-vehicleshop:client:SetCustomShowroomVeh', function(showroomVehicle, k)
    ...
    if QBCustom.ShowroomPositions[k].vehicle ~= showroomVehicle then
        ZyoCore.Functions.DeleteVehicle(GetClosestVehicle(QBCustom.ShowroomPositions[k].coords.x, QBCustom.ShowroomPositions[k].coords.y, QBCustom.ShowroomPositions[k].coords.z, 3.0, 0, 70))
...
AddEventHandler('qb-vehicleshop:client:ConfirmVehicle', function(Showroom, plate)
    ZyoCore.Functions.SpawnVehicle(Showroom.vehicle, function(veh)
        ...
        TriggerServerEvent("vehicletuning:server:SaveVehicleProps", ZyoCore.Functions.GetVehicleProperties(veh))
...
AddEventHandler('qb-vehicleshop:client:DoTestrit', function(plate)
    if ClosestCustomVehicle ~= 0 then
        ZyoCore.Functions.SpawnVehicle(QBCustom.ShowroomPositions[ClosestCustomVehicle].vehicle, function(veh)
            TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
            exports['LegacyFuel']:SetFuel(veh, 100)
            SetVehicleNumberPlateText(veh, plate)
            SetEntityAsMissionEntity(veh, true, true)
...
AddEventHandler('qb-vehicleshop:client:SellCustomVehicle', function(TargetId)
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if VehicleDist < 2.5 then
            ...
            ZyoCore.Functions.Notify("You are not near the vehicle!", "error")
        end
    else
        ZyoCore.Functions.Notify("There is no one around!", "error")
    end
end)
