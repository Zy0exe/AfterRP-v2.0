...
AddEventHandler('qb-vehicleshop:server:buyVehicle', function(vehicleData, garage)
    ...
    local vData = ZyoCore.Shared.Vehicles[vehicleData["model"]]
    local balance = pData.PlayerData.money["bank"]
    
    if (balance - vData["price"]) >= 0 then
        local plate = GeneratePlate()
        ZyoCore.Functions.ExecuteSql(false, "INSERT INTO `player_vehicles` (`steam`, `citizenid`, `vehicle`, `hash`, `mods`, `plate`, `garage`) VALUES ('"..pData.PlayerData.steam.."', '"..cid.."', '"..vData["model"].."', '"..GetHashKey(vData["model"]).."', '{}', '"..plate.."', '"..garage.."')")
        TriggerClientEvent("ZyoCore:Notify", src, "Bem feito! Seu veículo foi entregue "..QB.GarageLabel[garage], "success", 5000)
		...
		TriggerClientEvent("ZyoCore:Notify", src, "Você não tem dinheiro suficiente, falta €"..format_thousand(vData["price"] - balance), "error", 5000)
    end
end)
...
AddEventHandler('qb-vehicleshop:server:buyShowroomVehicle', function(vehicle, class)
    ...
    local cid = pData.PlayerData.citizenid
    local balance = pData.PlayerData.money["bank"]
    local vehiclePrice = ZyoCore.Shared.Vehicles[vehicle]["price"]
    local plate = GeneratePlate()

    if (balance - vehiclePrice) >= 0 then
        ZyoCore.Functions.ExecuteSql(false, "INSERT INTO `player_vehicles` (`steam`, `citizenid`, `vehicle`, `hash`, `mods`, `plate`, `state`) VALUES ('"..pData.PlayerData.steam.."', '"..cid.."', '"..vehicle.."', '"..GetHashKey(vehicle).."', '{}', '"..plate.."', 0)")
        TriggerClientEvent("ZyoCore:Notify", src, "Bem feito! Seu veículo está esperando por você lá fora.", "success", 5000)
        TriggerClientEvent('qb-vehicleshop:client:buyShowroomVehicle', src, vehicle, plate)
        pData.Functions.RemoveMoney('bank', vehiclePrice, "vehicle-bought-in-showroom")
        TriggerEvent("qb-log:server:sendLog", cid, "vehiclebought", {model=vehicle, name=ZyoCore.Shared.Vehicles[vehicle]["name"], from="showroom", moneyType="bank", price=ZyoCore.Shared.Vehicles[vehicle]["price"], plate=plate})
        TriggerEvent("qb-log:server:CreateLog", "vehicleshop", "Veículo comprado (Stand)", "green", "**"..GetPlayerName(src) .. "** Compraste um  " .. ZyoCore.Shared.Vehicles[vehicle]["name"] .. " por €" .. ZyoCore.Shared.Vehicles[vehicle]["price"])
    else
        TriggerClientEvent("ZyoCore:Notify", src, "Você não tem dinheiro suficiente, falta €"..format_thousand(vehiclePrice - balance), "error", 5000)
    end
end)
...
ZyoCore.Commands.Add("venda", "Vender o veículo em uma concessionária de carros personalizados", {}, false, function(source, args)
    ...
    if Player.PlayerData.job.name == "cardealer" then
        if TargetId ~= nil then
            ...
            TriggerClientEvent('ZyoCore:Notify', source, 'Você precisa dar o ID do jogador', 'error')
        end
    else
        TriggerClientEvent('ZyoCore:Notify', source, 'Você não é um revendedor de automóveis', 'error')
    end
end)

ZyoCore.Commands.Add("testdrive", "Faça um test drive", {}, false, function(source, args)
    local Player = ZyoCore.Functions.GetPlayer(source)
    local TargetId = args[1]
    ...
    if Player.PlayerData.job.name == "cardealer" then
        ...
        TriggerClientEvent('ZyoCore:Notify', source, 'Você chega a uma concessionária de automóveis', 'error')
    end
end)
...
AddEventHandler('qb-vehicleshop:server:ConfirmVehicle', function(ShowroomVehicle)
    ...
    local VehPrice = ZyoCore.Shared.Vehicles[ShowroomVehicle.vehicle].price
    local plate = GeneratePlate()
    ...
    if Player.PlayerData.money.cash >= VehPrice then
        ...
        ZyoCore.Functions.ExecuteSql(false, "INSERT INTO `player_vehicles` (`steam`, `citizenid`, `vehicle`, `hash`, `mods`, `plate`, `state`) VALUES ('"..Player.PlayerData.steam.."', '"..Player.PlayerData.citizenid.."', '"..ShowroomVehicle.vehicle.."', '"..GetHashKey(ShowroomVehicle.vehicle).."', '{}', '"..plate.."', 0)")
    elseif Player.PlayerData.money.bank >= VehPrice then
        ...
        if Player.PlayerData.money.cash > Player.PlayerData.money.bank then
            TriggerClientEvent('ZyoCore:Notify', src, 'Você não tem dinheiro suficiente .. falta ('..(Player.PlayerData.money.cash - VehPrice)..',-)')
        else
            TriggerClientEvent('ZyoCore:Notify', src, 'Você não tem dinheiro suficiente .. falta ('..(Player.PlayerData.money.bank - VehPrice)..',-)')
        end
    end
end)

ZyoCore.Functions.CreateCallback('qb-vehicleshop:server:SellVehicle', function(source, cb, vehicle, plate)
    local VehicleData = ZyoCore.Shared.VehicleModels[vehicle]
    local src = source
    local Player = ZyoCore.Functions.GetPlayer(src)

    ZyoCore.Functions.ExecuteSql(false, "SELECT * FROM `player_vehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `plate` = '"..plate.."'", function(result)
        if result[1] ~= nil then
            Player.Functions.AddMoney('bank', math.ceil(VehicleData["price"] / 100 * 60))
            ZyoCore.Functions.ExecuteSql(false, "DELETE FROM `player_vehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `plate` = '"..plate.."'")
            cb(true)
        else
            cb(false)
        end
