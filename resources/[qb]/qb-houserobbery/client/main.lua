...
AddEventHandler('ZyoCore:Client:OnPlayerLoaded', function()
    ZyoCore.Functions.TriggerCallback('qb-houserobbery:server:GetHouseConfig', function(HouseConfig)
...
Citizen.CreateThread(function()
    Citizen.Wait(500)
    requiredItems = {
        [1] = {name = ZyoCore.Shared.Items["lockpick"]["name"], image = ZyoCore.Shared.Items["lockpick"]["image"]},
        [2] = {name = ZyoCore.Shared.Items["screwdriverset"]["name"], image = ZyoCore.Shared.Items["screwdriverset"]["image"]},
    }
    while true do
        inRange = false
                ...
                if (GetDistanceBetweenCoords(pos, Config.Houses[currentHouse]["coords"]["x"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["x"], Config.Houses[currentHouse]["coords"]["y"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["y"], Config.Houses[currentHouse]["coords"]["z"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["z"] - Config.MinZOffset, true) < 1) then
                    if not Config.Houses[currentHouse]["furniture"][k]["searched"] then
                        if not Config.Houses[currentHouse]["furniture"][k]["isBusy"] then
                            ...
                            DrawText3Ds(Config.Houses[currentHouse]["coords"]["x"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["x"], Config.Houses[currentHouse]["coords"]["y"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["y"], Config.Houses[currentHouse]["coords"]["z"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["z"] - Config.MinZOffset, 'A abrir gavetas..')
                        end
                    else
            ...
            if closestHouse ~= nil then
                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                    if not Config.Houses[closestHouse]["opened"] then
                        ...
                        ZyoCore.Functions.Notify('A porta ja esta aberta..', 'error', 3500)
                    end
                else
                    ZyoCore.Functions.Notify('Não existe policias suficientes..', 'error', 3500)
                end
            end
        else
            ZyoCore.Functions.TriggerCallback('ZyoCore:HasItem', function(result)
                if closestHouse ~= nil then
                    if result then
                        ...
                        ZyoCore.Functions.Notify('Parece que te falta uma caixa de ferramentas para isto...', 'error', 3500)
                    end
                end
            end, "screwdriverset")
        end
...
function PoliceCall()
    ...
    if math.random(1, 100) <= chance then
        ...
        if closestPed ~= nil then
            ...
            local gender = "Homem"
            if ZyoCore.Functions.GetPlayerData().charinfo.gender == 1 then
                gender = "Mulher"
            end
...
function lockpickFinish(success)
    if success then
        ...
        ZyoCore.Functions.Notify('Conseguiste!', 'success', 2500)
    else
        if usingAdvanced then
            local itemInfo = ZyoCore.Shared.Items["advancedlockpick"]
            if math.random(1, 100) < 20 then
                TriggerServerEvent("ZyoCore:Server:RemoveItem", "advancedlockpick", 1)
                TriggerEvent('inventory:client:ItemBox', itemInfo, "remove")
            end
        else
            local itemInfo = ZyoCore.Shared.Items["lockpick"]
            if math.random(1, 100) < 40 then
                TriggerServerEvent("ZyoCore:Server:RemoveItem", "lockpick", 1)
                TriggerEvent('inventory:client:ItemBox', itemInfo, "remove")
            end
        end
        
        ZyoCore.Functions.Notify('Falhaste..', 'error', 2500)
    end
end
