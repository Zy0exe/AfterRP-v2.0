    ...
    if OutsideVehicles ~= nil and next(OutsideVehicles) ~= nil then
        if OutsideVehicles[vehicle.plate] ~= nil then
            ...
            if not VehExists then
                ZyoCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                    ZyoCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                        ZyoCore.Functions.SetVehicleProperties(veh, properties)
                        enginePercent = round(vehicle.engine / 10, 0)
                        bodyPercent = round(vehicle.body / 10, 0)
                        currentFuel = vehicle.fuel

                        if vehicle.plate ~= nil then
                        	ZyoCore.Functions.DeleteVehicle(OutsideVehicles[vehicle.plate])
                        ...
                        ZyoCore.Functions.Notify("Retiraste o veiculo: Motor: " .. enginePercent .. "% Chassi: " .. bodyPercent.. "% Comb.: "..currentFuel.. "%", "primary", 4500)
                        closeMenuFull()
                ...
                if Engine < 40.0 then
                    ZyoCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                        ZyoCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                            ...
                            doCarDamage(veh, vehicle)
                            TriggerServerEvent('qb-garage:server:updateVehicleState', 0, vehicle.plate, vehicle.garage)
                            ZyoCore.Functions.Notify("Veiculo Desligado: Motor: " .. enginePercent .. "% Chassi: " .. bodyPercent.. "% Comb.: "..currentFuel.. "%", "primary", 4500)
                            closeMenuFull()
                            SetVehicleEngineOn(veh, true, true)
                            TriggerEvent('persistent-vehicles/register-vehicle', veh)
                        end, vehicle.plate)
                    ...
                    ZyoCore.Functions.Notify('Não podes duplicar este veiculo..', 'error')
...
function AddTemporaryBlip(vehicle)  
    ...
    local VehicleData = ZyoCore.Shared.VehicleModels[GetEntityModel(vehicle)]
    ...
    ZyoCore.Functions.Notify("O teu "..VehicleData["name"].." esta temporariamente marcado no GPS!", "success", 10000)

    SetTimeout(60 * 1000, function()
        ZyoCore.Functions.Notify('O teu veiculo deixou de estar marcado no GPS!', 'error')
        RemoveBlip(TempBlip)
    end)
end
...
function HouseGarage(house)
    ZyoCore.Functions.TriggerCallback("qb-garage:server:GetHouseVehicles", function(result)
        ped = GetPlayerPed(-1);
        MenuTitle = "Depot Vehicles :"
        ClearMenu()

        if result == nil then
            ZyoCore.Functions.Notify("You have no vehicles in your garage", "error", 5000)
            closeMenuFull()
        else
            ...
            for k, v in pairs(result) do
                ...
                local label = ZyoCore.Shared.Vehicles[v.vehicle]["name"]
                if ZyoCore.Shared.Vehicles[v.vehicle]["brand"] ~= nil then
                    label = ZyoCore.Shared.Vehicles[v.vehicle]["brand"].." "..ZyoCore.Shared.Vehicles[v.vehicle]["name"]
                end
...
function DepotLijst()
    ZyoCore.Functions.TriggerCallback("qb-garage:server:GetDepotVehicles", function(result)
        ped = GetPlayerPed(-1);
        MenuTitle = "Veiculos Apreendidos :"
        ClearMenu()

        if result == nil then
            ZyoCore.Functions.Notify("Não tens nenhum veiculo apreendido", "error", 5000)
            closeMenuFull()
        else
...
function VoertuigLijst()
    ZyoCore.Functions.TriggerCallback("qb-garage:server:GetUserVehicles", function(result)
        ped = GetPlayerPed(-1);
        MenuTitle = "Os meus veiculos :"
        ClearMenu()

        if result == nil then
            ZyoCore.Functions.Notify("Não tens nenhum veiculo nesta garagem", "error", 5000)
            closeMenuFull()
        else
...
function TakeOutVehicle(vehicle)
    ...
    if vehicle.state == "Garagem" then
        ...
        if jaTaSpawnadinho == true then
        	ZyoCore.Functions.Notify("Aguarda 2s até conseguires spawnar outro veiculo")
        	Wait(2000)
	        ...
	        ZyoCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
	            ZyoCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
	                ...
	                SetEntityAsMissionEntity(veh, true, true)
	                TriggerServerEvent('qb-garage:server:updateVehicleState', 0, vehicle.plate, vehicle.garage)
	                ZyoCore.Functions.Notify("Retiraste um veiculo: Motor: " .. enginePercent .. "% Chassi: " .. bodyPercent.. "% Comb.: "..currentFuel.. "%", "primary", 4500)
	                closeMenuFull()
	                jaTaSpawnadinho = false
	                TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
    ...
    elseif vehicle.state == "Fora" then
        ZyoCore.Functions.Notify("Tens a certeza que o teu veiculo esta nos apreendidos??", "error", 2500)
    elseif vehicle.state == "Apreendido" then
        ZyoCore.Functions.Notify("O teu veiculo foi apreendido pela policia", "error", 4000)
    end
end
            ...
            if putDist <= 25 and IsPedInAnyVehicle(ped) then
                ...
                if putDist <= 1.5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        ZyoCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
                            if owned then
                                local bodyDamage = math.ceil(GetVehicleBodyHealth(curVeh))
                                local engineDamage = math.ceil(GetVehicleEngineHealth(curVeh))
                                local totalFuel = exports['LegacyFuel']:GetFuel(curVeh)
                                
                                local propertiescarro = ZyoCore.Functions.GetVehicleProperties(curVeh)
                                ...
                                ZyoCore.Functions.Notify("Guardas-te o teu veiculo em, "..Garages[k].label, "primary", 4500)
                            else
                                ZyoCore.Functions.Notify("Este veiculo não te pertence...", "error", 3500)
                            end
                        end, plate)
                    end
                end
                    ...
                    if takeDist < 2.0 then
                        ...
                        elseif IsPedInAnyVehicle(ped) then
                            ...
                            if IsControlJustPressed(0, 38) then
                                local curVeh = GetVehiclePedIsIn(ped)
                                local plate = GetVehicleNumberPlateText(curVeh)
                                ZyoCore.Functions.TriggerCallback('qb-garage:server:checkVehicleHouseOwner', function(owned)
                                    if owned then
                                        ...
                                        ZyoCore.Functions.Notify("Guardas-te o veiculo em, "..HouseGarages[currentHouseGarage].label, "primary", 4500)
                                    else
                                        ZyoCore.Functions.Notify("Este veiculo não é propriedade de ninguem...", "error", 3500)
                                    end
