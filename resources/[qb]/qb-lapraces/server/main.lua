...
Citizen.CreateThread(function()
    ZyoCore.Functions.ExecuteSql(false, "SELECT * FROM `lapraces`", function(races)
        if races[1] ~= nil then
...
ZyoCore.Functions.CreateCallback('qb-lapraces:server:GetRacingLeaderboards', function(source, cb)
    cb(Races)
end)
...
AddEventHandler('qb-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            ZyoCore.Functions.ExecuteSql(false, "UPDATE `lapraces` SET `records` = '"..json.encode(Races[RaceData.RaceId].Records).."' WHERE `raceid` = '"..RaceData.RaceId.."'")
            TriggerClientEvent('qb-phone_new:client:RaceNotify', src, 'You have broken the WR on '..RaceData.RaceName..' with a time of: '..SecondsToClock(BLap)..'!')
        end
    else
        Races[RaceData.RaceId].Records = {
            Time = BLap,
        ...
        ZyoCore.Functions.ExecuteSql(false, "UPDATE `lapraces` SET `records` = '"..json.encode(Races[RaceData.RaceId].Records).."' WHERE `raceid` = '"..RaceData.RaceId.."'")
        TriggerClientEvent('qb-phone_new:client:RaceNotify', src, 'You have set the WR on '..RaceData.RaceName..' with a time of: '..SecondsToClock(BLap)..'!')
    end
    ...
    TriggerClientEvent('qb-phone_new:client:UpdateLapraces', -1)
end)
...
function IsWhitelisted(CitizenId)
    ...
    local Perms = ZyoCore.Functions.GetPermission(Player.PlayerData.source)
...
AddEventHandler('qb-lapraces:server:CreateLapRace', function(RaceName)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            ...
            TriggerClientEvent('ZyoCore:Notify', source, 'There is already a rave whit that name.', 'error')
        end
    else
        TriggerClientEvent('ZyoCore:Notify', source, 'You dont have the rights to create a race.', 'error')
    end
end)

ZyoCore.Functions.CreateCallback('qb-lapraces:server:GetRaces', function(source, cb)
    cb(AvailableRaces)
end)

ZyoCore.Functions.CreateCallback('qb-lapraces:server:GetListedRaces', function(source, cb)
    cb(Races)
end)

ZyoCore.Functions.CreateCallback('qb-lapraces:server:GetRacingData', function(source, cb, RaceId)
    cb(Races[RaceId])
end)

ZyoCore.Functions.CreateCallback('qb-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(ZyoCore.Functions.GetPlayer(source).PlayerData.citizenid))
end)

ZyoCore.Functions.CreateCallback('qb-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
    cb(IsWhitelisted(ZyoCore.Functions.GetPlayer(source).PlayerData.citizenid), IsNameAvailable(TrackName))
end)
...
ZyoCore.Functions.CreateCallback('qb-lapraces:server:GetTrackData', function(source, cb, RaceId)
    ZyoCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `citizenid` = '"..Races[RaceId].Creator.."'", function(result)
...
AddEventHandler('qb-lapraces:server:JoinRace', function(RaceData)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('ZyoCore:Notify', src, 'You were the only one in the race the race wil end.', 'error')
    ...
    TriggerClientEvent('qb-phone_new:client:UpdateLapraces', -1)
    local creatorsource = ZyoCore.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData.source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('qb-phone_new:client:RaceNotify', creatorsource, string.sub(Player.PlayerData.charinfo.firstname, 1, 1)..'. '..Player.PlayerData.charinfo.lastname..' is de race gejoined!')
    end
end)
...
AddEventHandler('qb-lapraces:server:LeaveRace', function(RaceData)
    ...
    local creatorsource = ZyoCore.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData.source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('qb-phone_new:client:RaceNotify', creatorsource, string.sub(Player.PlayerData.charinfo.firstname, 1, 1)..'. '..Player.PlayerData.charinfo.lastname..' is de race geleaved!')
    end
    local AmountOfRacers = 0
    for k, v in pairs(Races[RaceData.RaceId].Racers) do
        AmountOfRacers = AmountOfRacers + 1
            ...
            if not Races[RaceId].Started then
                ...
                SetTimeout(5 * 60 * 1000, function()
                    if Races[RaceId].Waiting then
                        ...
                        for cid,_ in pairs(Races[RaceId].Racers) do
                            local RacerData = ZyoCore.Functions.GetPlayerByCitizenId(cid)
                ...
                TriggerClientEvent('ZyoCore:Notify', source, 'The race is already active...', 'error')
            end
        else
            TriggerClientEvent('ZyoCore:Notify', source, 'The race is already active..', 'error')
        end
    else
        TriggerClientEvent('ZyoCore:Notify', source, 'This race does not exsist :(', 'error')
    end
end)
...
AddEventHandler('qb-lapraces:server:StartRace', function(RaceId)
    local src = source
    local MyPlayer = ZyoCore.Functions.GetPlayer(src)
    local AvailableKey = GetOpenedRaceKey(RaceId)
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            TriggerClientEvent('ZyoCore:Notify', src, 'Your not the creator of the race..', 'error')
        end
    else
        TriggerClientEvent('ZyoCore:Notify', src, 'You are not in a race..', 'error')
    end
end)
...
AddEventHandler('qb-lapraces:server:SaveRace', function(RaceData)
    ...
    ZyoCore.Functions.ExecuteSql(false, "INSERT INTO `lapraces` (`name`, `checkpoints`, `creator`, `distance`, `raceid`) VALUES ('"..RaceData.RaceName.."', '"..json.encode(Checkpoints).."', '"..Player.PlayerData.citizenid.."', '"..RaceData.RaceDistance.."', '"..GenerateRaceId().."')")
end)
...
ZyoCore.Commands.Add("togglesetup", "Zet Racing setup aan/uit", {}, false, function(source, args)
    local Player = ZyoCore.Functions.GetPlayer(source)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        Config.RaceSetupAllowed = not Config.RaceSetupAllowed
        if not Config.RaceSetupAllowed then
            TriggerClientEvent('ZyoCore:Notify', source, 'There cant be made any more races', 'error')
        else
            TriggerClientEvent('ZyoCore:Notify', source, 'You can make races again!', 'success')
        end
    else
        TriggerClientEvent('ZyoCore:Notify', source, 'You dont have the rights to do this.', 'error')
    end
end)

ZyoCore.Commands.Add("cancelrace", "Cancel gaande race..", {}, false, function(source, args)
    local Player = ZyoCore.Functions.GetPlayer(source)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        ...
        if RaceName ~= nil then
            ...
            if Races[RaceId].Started then
                ...
                TriggerClientEvent('ZyoCore:Notify', source, 'This race has not started yet.', 'error')
            end
        end
    else
        TriggerClientEvent('ZyoCore:Notify', source, 'You dont have rights to do this', 'error')
    end
end)

ZyoCore.Functions.CreateCallback('qb-lapraces:server:CanRaceSetup', function(source, cb)
