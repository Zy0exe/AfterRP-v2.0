	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not isPickingUp then
				ZyoCore.Functions.Draw2DText(0.5, 0.88, 'Carrega ~g~[E]~w~ para pegar Cannabis', 0.5)
			end
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				ZyoCore.Functions.Progressbar("search_register", "A pegar Cannabis..", 7500, false, true, {
					...
					ZyoCore.Functions.DeleteObject(nearbyObject)
...
AddEventHandler('onResourceStop', function(resource) --weedPlants
	if resource == GetCurrentResourceName() then
		for k, v in pairs(weedPlants) do
			ZyoCore.Functions.DeleteObject(v)
		end
	end
end)
function SpawnWeedPlants() --This spawns in the Weed plants, 
	while spawnedWeed < 25 do
		...
		ZyoCore.Functions.SpawnLocalObject('prop_weed_02', weedCoords, function(obj) --- change this prop to whatever plant you are trying to use 
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.WeedProcessing.coords, true) < 1 then
			...
			if not isProcessing then
				ZyoCore.Functions.DrawText3D(Config.CircleZones.WeedProcessing.coords.x, Config.CircleZones.WeedProcessing.coords.y, Config.CircleZones.WeedProcessing.coords.z, 'Carrega ~g~[ E ]~w~ para processar')
			end
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				...
				if (hasWeed and hasBag) then
					Processweed()
				elseif (hasWeed) then
					ZyoCore.Functions.Notify('Você não tem sacolas plásticas suficientes.', 'error')
				elseif (hasBag) then
					ZyoCore.Functions.Notify('Você não tem cannabis suficiente.', 'error')
				else
					ZyoCore.Functions.Notify('Você não tem cannabis ou sacolas plásticas suficientes.', 'error')
				end
			end
		else
			Citizen.Wait(500)
...
function Processweed()  -- simple animations to loop while process is taking place
	...
	ZyoCore.Functions.Progressbar("search_register", "Tentando processar..", 15000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer.coords, true) < 1 then
			...
			if not isProcessing2 then
				ZyoCore.Functions.DrawText3D(Config.CircleZones.DrugDealer.coords.x, Config.CircleZones.DrugDealer.coords.y, Config.CircleZones.DrugDealer.coords.z, 'Carrega ~g~[ E ]~w~ para vender')
			end
