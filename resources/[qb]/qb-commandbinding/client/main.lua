        ...
        if isLoggedIn then
            for k, v in pairs(availableKeys) do
                if IsControlJustPressed(0, Keys[v]) or IsDisabledControlJustPressed(0, Keys[v]) then
                    local keyMeta = ZyoCore.Functions.GetPlayerData().metadata["commandbinds"]
                    local args = {}
                    if next(keyMeta) ~= nil then
                        if keyMeta[v]["command"] ~= "" then
                            ...
                            TriggerServerEvent('ZyoCore:CallCommand', keyMeta[v]["command"], args)
                            keyPressed = true
                        else
                            ZyoCore.Functions.Notify('Ainda não tens nada bindado ao ['..v..'] , /binds para bindares um comando', 'primary', 4000)
                        end
                    else
                        ZyoCore.Functions.Notify('Indica um comando, /binds para bindares', 'primary', 4000)
                    end
                end
            end
...
RegisterNUICallback('save', function(data)
    ...
    ZyoCore.Functions.Notify('Salvaste as tuas binds!', 'success')
