...
AddEventHandler('hospital:client:TakeBrancard', function()
    ...
    if Vehicle ~= 0 then
        ...
        if Obj ~= nil or Obj ~= 0 then
            ...
            ZyoCore.Functions.Notify("Algo deu errado meu amigo..", 'error')
        end
    else
        ZyoCore.Functions.Notify("Você não está perto de uma ambulância..", 'error')
    end
end)
...
AddEventHandler('hospital:client:RemoveBrancard', function()
    ...
    if BrancardObject ~= nil then
        ...
        if Dist < 3.0 then
            ...
            ZyoCore.Functions.Notify('Estas muito longe!', 'error')
        end
    end
end)
...
AddEventHandler('qb-radialmenu:client:Result', function(IsBusy, type)
    ...
    if type == "lay" then
        if not IsBusy then
            ...
            ZyoCore.Functions.Notify("Esta maca ja esta em uso!", "error")
            IsLayingOnBed = false
        end
    else
        if not IsBusy then
