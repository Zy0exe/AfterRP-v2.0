...
AddEventHandler('qb-radialmenu:client:noPlayers', function(data)
    ZyoCore.Functions.Notify('Ninguem por perto', 'error', 2500)
end)
...
AddEventHandler('qb-radialmenu:client:openDoor', function(data)
    ...
    if closestVehicle ~= 0 then
        ...
        ZyoCore.Functions.Notify('Nenhum veiculo por perto...', 'error', 2500)
    end
end)
        ...
        if GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) then
            if DoesExtraExist(veh, extra) then 
                if IsVehicleExtraTurnedOn(veh, extra) then
                    ...
                    ZyoCore.Functions.Notify('Extra ' .. extra .. ' Desativado', 'error', 2500)
                else
                    enginehealth = GetVehicleEngineHealth(veh)
                    bodydamage = GetVehicleBodyHealth(veh)
                    ...
                    SetVehicleEngineHealth(veh, enginehealth)
                    SetVehicleBodyHealth(veh, bodydamage)
                    ZyoCore.Functions.Notify('Extra ' .. extra .. ' Ativado', 'success', 2500)
                end    
            else
                ZyoCore.Functions.Notify('Extra ' .. extra .. ' Não existe neste veiculo ', 'error', 2500)
            end
        else
            ZyoCore.Functions.Notify('Não és o condutor do veiculo !', 'error', 2500)
        end
    end
end)
    ...
    if not HasHarnass then
        ...
        if IsSeatFree then
            if kmh <= 100.0 then
                ...
                ZyoCore.Functions.Notify('Mudas-te de assento para  '..data.title..'!')
            else
                ZyoCore.Functions.Notify('Diminui a velocidade para alterares de assento..')
            end
        else
            ZyoCore.Functions.Notify('Esse lugar esta ocupado..')
        end
    else
        ZyoCore.Functions.Notify('Tens um cinto de 3 pontas, não consegues alterar o lugar...', 'error')
    end
end)
