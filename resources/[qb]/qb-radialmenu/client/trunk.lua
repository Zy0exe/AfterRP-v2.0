...
AddEventHandler('qb-trunk:client:KidnapTrunk', function()
    closestPlayer, distance = ZyoCore.Functions.GetClosestPlayer()
    ...
    if (distance ~= -1 and distance < 2) then
        if isKidnapping then
            ...
            ZyoCore.Functions.Notify('Essa pessoa não esta a ser raptada!', 'error')
        end
    end
end)
...
AddEventHandler('qb-trunk:client:KidnapGetIn', function(veh)
    ...
    if Config.TrunkClasses[vehClass].allowed then
        ZyoCore.Functions.TriggerCallback('qb-trunk:server:getTrunkBusy', function(isBusy)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            -- if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                ZyoCore.Functions.Notify('Entras-te na bagageira.', 'success', 4000)
                                TrunkCam(true)

                                isKidnapped = true
                            -- else
                                -- ZyoCore.Functions.Notify('Is de kofferbak dicht?', 'error', 2500)
                            -- end
                        else
                            local ped = GetPlayerPed(-1)
                            local vehicle = GetEntityAttachedTo(PlayerPedId())
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                ZyoCore.Functions.Notify('A bagageira secalhar esta fechada', 'error', 2500)
                            end
                        end
                    else
                        ZyoCore.Functions.Notify('Será que ainda tem alguem na mala?', 'error', 2500)
                    end 
                else
                    ZyoCore.Functions.Notify('Ja estas na Bagageira', 'error', 2500)
                end 
            else
                ZyoCore.Functions.Notify('Não consegues entrar nesta bagageira..', 'error', 2500)
            end
        end, plate)
    else
        ZyoCore.Functions.Notify('Não consegues entrar nesta bagageira..', 'error', 2500)
                    ...
                    if not inTrunk then
                        if not isBusy then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                Citizen.Wait(500)
                                SetVehicleDoorShut(closestVehicle, 5, false)
                                ZyoCore.Functions.Notify('Ja estas na bagageira.', 'Goodluck', 4000)
                                TrunkCam(true)
                            else
                                ZyoCore.Functions.Notify('A bagageira secalhar esta fechada', 'error', 2500)
                            end
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                            ...
                            ZyoCore.Functions.Notify('Será que a bagageira esta trancada?', 'error', 2500)
                        end
                    end
