...
AddEventHandler("qb-pawnshop:server:sellPawnItems", function()
    ...
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        ...
        TriggerClientEvent('ZyoCore:Notify', src, "You have sold your items")
    end
end)
...
AddEventHandler("qb-pawnshop:server:getGoldBars", function()
    ...
    if GoldBarsAmount > 0 then
        if Player.Functions.AddItem("goldbar", GoldBarsAmount) then
            GoldBarsAmount = 0
            TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items["goldbar"], "add")
            Config.IsMelting = false
            Config.CanTake = false
            Config.MeltTime = 300
            TriggerClientEvent("qb-pawnshop:client:SetTakeState", -1, false)
        else
            TriggerClientEvent('ZyoCore:Notify', src, "You dont have any space in your inventory", "error")
        end
    end
end)
...
AddEventHandler("qb-pawnshop:server:sellGold", function()
    ...
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        ...
        TriggerClientEvent('ZyoCore:Notify', src, "You have sold your item's")
    end
end)
        ...
        for k, v in pairs(Player.PlayerData.items) do 
            if Player.PlayerData.items[k] ~= nil then 
                if MeltItems[Player.PlayerData.items[k].name] ~= nil then 
                    ...
                    if amount < 1 then
                        TriggerClientEvent('ZyoCore:Notify', src, "You dont have enough " .. Player.PlayerData.items[k].label, "error")
                    else
...
ZyoCore.Functions.CreateCallback('qb-pawnshop:server:getSellPrice', function(source, cb)
    local retval = 0
...
ZyoCore.Functions.CreateCallback('qb-pawnshop:melting:server:GetConfig', function(source, cb)
    cb(Config.IsMelting, Config.MeltTime, Config.CanTake)
end)

ZyoCore.Functions.CreateCallback('qb-pawnshop:server:getSellHardwarePrice', function(source, cb)
    local retval = 0
    local amount = 0
    local Player = ZyoCore.Functions.GetPlayer(source)
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
...
ZyoCore.Functions.CreateCallback('qb-pawnshop:server:hasGold', function(source, cb)
	local retval = false
    local Player = ZyoCore.Functions.GetPlayer(source)
