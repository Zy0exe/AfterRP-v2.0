...
AddEventHandler("ZyoCore:Client:OnPlayerLoaded", function()
    ZyoCore.Functions.TriggerCallback('qb-pawnshop:melting:server:GetConfig', function(IsMelting, MeltTime, CanTake)
                ...
                if not Config.IsMelting then
                    if Config.CanTake then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then 
                            ...
                            ScrapAnim(1000)
                            ZyoCore.Functions.Progressbar("drop_golden_stuff", "Picking up items..", 1000, false, true, {
                ...
                if GetClockHours() >= 9 and GetClockHours() <= 18 then
                    ...
                    elseif sellItemsSet and hasGold then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            ...
                            ZyoCore.Functions.Progressbar("sell_gold", "Sell gold..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                openingDoor = false
                                ClearPedTasks(GetPlayerPed(-1))
                                ZyoCore.Functions.Notify("Process canceled..", "error")
                            end)
                        end
                    else
...
function HasPlayerGold()
	local retval = false
	ZyoCore.Functions.TriggerCallback('qb-pawnshop:server:hasGold', function(result)
		retval = result
	end)
    Citizen.Wait(500)
	return retval
