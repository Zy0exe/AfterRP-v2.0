...
AddEventHandler('qb-multicharacter:server:disconnect', function()
    ...
    DropPlayer(src, "Você se desconectou do AFTER V2.0")
end)
...
AddEventHandler('qb-multicharacter:server:loadUserData', function(cData)
    local src = source
    if ZyoCore.Player.Login(src, cData.citizenid) then
        print('^2[jk-core]^7 '..GetPlayerName(src)..' (Citizen ID: '..cData.citizenid..') foi carregado com sucesso!')
        ZyoCore.Commands.Refresh(src)
        loadHouseData()
		--TriggerEvent('ZyoCore:Server:OnPlayerLoaded')-
        --TriggerClientEvent('ZyoCore:Client:OnPlayerLoaded', src)
...
AddEventHandler('qb-multicharacter:server:createCharacter', function(data)
    ...
    --ZyoCore.Player.CreateCharacter(src, data)
    if ZyoCore.Player.Login(src, false, newData) then
...
function GiveStarterItems(source)
    ...
    for k, v in pairs(ZyoCore.Shared.StarterItems) do
        local info = {}
...
AddEventHandler('qb-multicharacter:server:deleteCharacter', function(citizenid)
    local src = source
    ZyoCore.Player.DeleteCharacter(src, citizenid)
end)

ZyoCore.Functions.CreateCallback("qb-multicharacter:server:GetUserCharacters", function(source, cb)
...
ZyoCore.Functions.CreateCallback("qb-multicharacter:server:GetServerLogs", function(source, cb)
    exports['ghmattimysql']:execute('SELECT * FROM server_logs', function(result)
        cb(result)
    end)
end)

ZyoCore.Functions.CreateCallback("test:yeet", function(source, cb)
...
ZyoCore.Commands.Add("char", "Give item to a player", {{name="id", help="Player ID"},{name="item", help="Name of the item (not a label)"}, {name="amount", help="Amount of items"}}, false, function(source, args)
    ZyoCore.Player.Logout(source)
    TriggerClientEvent('qb-multicharacter:client:chooseChar', source)
end, "admin")

ZyoCore.Commands.Add("closeNUI", "Give item to a player", {{name="id", help="Player ID"},{name="item", help="Name of the item (not a label)"}, {name="amount", help="Amount of items"}}, false, function(source, args)
    TriggerClientEvent('qb-multicharacter:client:closeNUI', source)
end)

ZyoCore.Functions.CreateCallback("qb-multicharacter:server:getSkin", function(source, cb, cid)
    local src = source

    ZyoCore.Functions.ExecuteSql(false, "SELECT * FROM `playerskins` WHERE `citizenid` = '"..cid.."' AND `active` = 1", function(result)
