...
RegisterNUICallback('chooseAppa', function(data)
    ...
    TriggerServerEvent('ZyoCore:Server:OnPlayerLoaded')
    TriggerEvent('ZyoCore:Client:OnPlayerLoaded')
...
AddEventHandler('qb-spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        ZyoCore.Functions.TriggerCallback('qb-spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
