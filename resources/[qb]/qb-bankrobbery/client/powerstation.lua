        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    ZyoCore.Functions.Notify("Parece que os fusiveis queimaram..", "error")
                end
            else
...
RegisterNUICallback('thermitefailed', function()
    ...
    TriggerServerEvent("ZyoCore:Server:RemoveItem", "thermite", 1)
    TriggerEvent('inventory:client:ItemBox', ZyoCore.Shared.Items["thermite"], "remove")
...
RegisterNUICallback('thermitesuccess', function()
    ...
    local coords = GetEntityCoords(GetPlayerPed(-1))
    while time > 0 do 
        ZyoCore.Functions.Notify("Irá começar em " .. time .. "..")
        Citizen.Wait(1000)
        time = time - 1
    end
    local randTime = math.random(10000, 15000)
    ...
    if currentStation ~= 0 then
        ZyoCore.Functions.Notify("Os fusiveis foram partidos", "success")
        TriggerServerEvent("qb-bankrobbery:server:SetStationStatus", currentStation, true)
    elseif currentGate ~= 0 then
        ZyoCore.Functions.Notify("A porta esta agora aberta", "success")
