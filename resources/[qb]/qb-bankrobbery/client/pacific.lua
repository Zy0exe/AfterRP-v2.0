...
Citizen.CreateThread(function()
    ...
    local requiredItems3 = {
        [1] = {name = ZyoCore.Shared.Items["thermite"]["name"], image = ZyoCore.Shared.Items["thermite"]["image"]},
    }
    ...
    local requiredItems = {
        [1] = {name = ZyoCore.Shared.Items["security_card_02"]["name"], image = ZyoCore.Shared.Items["security_card_02"]["image"]},
    }
    while true do
                    ...
                    if CurrentCops >= Config.MinimumPacificPolice then
                        if not Config.BigBanks["pacific"]["isOpened"] then 
                            ZyoCore.Functions.TriggerCallback('ZyoCore:HasItem', function(result)
                                if result then 
                                    TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                                    ZyoCore.Functions.Progressbar("hack_gate", "Connecting electronic kit..", math.random(5000, 10000), false, true, {
        ...
        ZyoCore.Functions.TriggerCallback('qb-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPacificPolice then
                    if not Config.BigBanks["pacific"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems2, false)
                        ZyoCore.Functions.Progressbar("security_pass", "A digitar a Palavra-Passe..", math.random(5000, 10000), false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            TriggerServerEvent("ZyoCore:Server:RemoveItem", "security_card_02", 1)
                            if not copsCalled then
                                local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, pos.x, pos.y, pos.z, Citizen.PointerValueInt(), Citizen.PointerValueInt())
                                local street1 = GetStreetNameFromHashKey(s1)
                                local street2 = GetStreetNameFromHashKey(s2)
