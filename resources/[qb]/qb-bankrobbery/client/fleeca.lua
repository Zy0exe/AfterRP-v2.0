...
AddEventHandler("ZyoCore:Client:OnPlayerLoaded", function()
    ...
    ZyoCore.Functions.TriggerCallback('qb-bankrobbery:server:GetConfig', function(config)
        Config = config
    end)
    onDuty = true
    ResetBankDoors()
...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = ZyoCore.Shared.Items["electronickit"]["name"], image = ZyoCore.Shared.Items["electronickit"]["image"]},
        [2] = {name = ZyoCore.Shared.Items["trojan_usb"]["name"], image = ZyoCore.Shared.Items["trojan_usb"]["image"]},
    }
    while true do
        local ped = GetPlayerPed(-1)
        local pos = GetEntityCoords(ped)
                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, Keys["E"]) then
                                            if CurrentCops >= Config.MinimumFleecaPolice then
                                                ...
                                                ZyoCore.Functions.Notify("Não existe policias suficientes..", "error")
                                            end
                                        end
                                    end
                                end
...
AddEventHandler('electronickit:UseElectronickit', function()
    ...
    if closestBank ~= nil then
        ZyoCore.Functions.TriggerCallback('qb-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if closestBank ~= nil then
                    local dist = GetDistanceBetweenCoords(pos, Config.SmallBanks[closestBank]["coords"]["x"], Config.SmallBanks[closestBank]["coords"]["y"], Config.SmallBanks[closestBank]["coords"]["z"])
                    if dist < 1.5 then
                        if CurrentCops >= Config.MinimumFleecaPolice then
                            if not Config.SmallBanks[closestBank]["isOpened"] then 
                                ZyoCore.Functions.TriggerCallback('ZyoCore:HasItem', function(result)
                                    if result then 
                                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                                        ZyoCore.Functions.Progressbar("hack_gate", "A utilizar um Kit eletronico..", math.random(5000, 10000), false, true, {
                                        ...
                                        ZyoCore.Functions.Notify("Esta a faltar-te um item..", "error")
                                    end
                                end, "trojan_usb")
                            else
                                ZyoCore.Functions.Notify("Parece que o banco ja foi arrombado..", "error")
                            end
                        else
                            ZyoCore.Functions.Notify("Não existe policias suficientes..", "error")
                        end
                ...
                ZyoCore.Functions.Notify("O codigo de segurança esta ativo,de momento não é possivel arrombar a porta..", "error", 5500)
            end
        end)
    end
end)
...
function openLocker(bankId, lockerId)
    ...
    if bankId == "paleto" then
        ZyoCore.Functions.TriggerCallback('qb-radio:server:GetItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                ZyoCore.Functions.Progressbar("open_locker_drill", "A arrombar o cofre...", math.random(40000, 60000), false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    ZyoCore.Functions.Notify("Sucesso!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(GetPlayerPed(-1), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                    TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                ...
                ZyoCore.Functions.Notify("It looks like the safe lock is too strong..", "error")
                TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    elseif bankId == "pacific" then
        ZyoCore.Functions.TriggerCallback('qb-radio:server:GetItem', function(hasItem)
            if hasItem then
                ...
                AttachEntityToEntity(DrillObject, GetPlayerPed(-1), GetPedBoneIndex(GetPlayerPed(-1), 57005), 0.14, 0, -0.01, 90.0, -90.0, 180.0, true, true, false, true, 1, true)
                IsDrilling = true
                ZyoCore.Functions.Progressbar("open_locker_drill", "Cracking the save..", math.random(40000, 60000), false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
                ...
                ZyoCore.Functions.Notify("Parece que o cadeado do cofre é muito forte...", "error")
                TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    else
        IsDrilling = true
        ZyoCore.Functions.Progressbar("open_locker", "A arrombar o cofre..", math.random(24000, 48000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
    ...
    if PlayerJob.name == "police" and onDuty then 
        ...
        if type == "small" then
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                ...
                callSign = ZyoCore.Functions.GetPlayerData().metadata["callsign"],
            })
