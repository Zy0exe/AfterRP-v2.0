...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = ZyoCore.Shared.Items["security_card_01"]["name"], image = ZyoCore.Shared.Items["security_card_01"]["image"]},
    }
        ...
        ZyoCore.Functions.TriggerCallback('qb-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                        ZyoCore.Functions.Progressbar("security_pass", "Validitating card..", math.random(5000, 10000), false, true, {
                            ...
                            TriggerServerEvent("ZyoCore:Server:RemoveItem", "security_card_01", 1)
