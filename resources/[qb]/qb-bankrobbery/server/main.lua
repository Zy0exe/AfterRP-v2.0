        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == "item" then
                    ...
                    TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items[item.item], "add")
                elseif Config.RewardTypes[itemType].type == "money" then
                    local info = {
                        worth = math.random(4000, 6000)
                    }
                    ply.Functions.AddItem('markedbills', 1, false, info)
                    TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items['markedbills'], "add")
                end
            else
                ply.Functions.AddItem('security_card_01', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items['security_card_01'], "add")
            end
        else
            ply.Functions.AddItem('weapon_stungun', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items['weapon_stungun'], "add")
        end
    elseif type == "paleto" then
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items['security_card_02'], "add")
            end
        else
            ply.Functions.AddItem('weapon_vintagepistol', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items['weapon_vintagepistol'], "add")
        end
    elseif type == "pacific" then
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items['cryptostick'], "add")
            end
        else
            ...
            if chance == odd then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items['weapon_microsmg'], "add")
            else
                ply.Functions.AddItem('weapon_minismg', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, ZyoCore.Shared.Items['weapon_minismg'], "add")
            end
        end
    end
end)

ZyoCore.Functions.CreateCallback('qb-bankrobbery:server:isRobberyActive', function(source, cb)
    cb(robberyBusy)
end)

ZyoCore.Functions.CreateCallback('qb-bankrobbery:server:GetConfig', function(source, cb)
    cb(Config)
end)
...
ZyoCore.Functions.CreateUseableItem("thermite", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('lighter') ~= nil then
        TriggerClientEvent("thermite:UseThermite", source)
    else
        TriggerClientEvent('ZyoCore:Notify', source, "Você está faltando algo para acender a termite..", "error")
    end
end)

ZyoCore.Functions.CreateUseableItem("security_card_01", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
...
ZyoCore.Functions.CreateUseableItem("security_card_02", function(source, item)
    local Player = ZyoCore.Functions.GetPlayer(source)
